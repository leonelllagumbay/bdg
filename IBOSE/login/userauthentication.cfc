component  displayname="userauthentication" hint="Handles all the user authentication"
{
	...
	public struct function changeUserPwdNative(required string theoldhashedpassword,required string theusername, required string thepassword) {
		...
		if(Tresult == "true") {
			...
			queryService.addParam(name="cdate",value="#CreateODBCDateTime(now())#",cfsqltype="cf_sql_timestamp");
	...
	public struct function validateUserNative(required string theusername, required string thepassword) hint="check if the user is a valid user" {
		...
		if(CheckedUser.recordcount > 0) { /* User has initially valid login */
				...
				if(CheckedUser[ "DISABLEACCOUNT" ][ cnt ] == 'N') {
					...
					if(CheckedUser.PWDNEVEREXPIRES != true) {
						...
						if (IsDate(CheckedUser.DATEPASSWORD)) {
							baseDate = CheckedUser.DATEPASSWORD;
							cresult = validationObj.checkIfPwdIsExpired(CheckedUser.MAXPASSWORDAGE,baseDate);
							if(cresult == -1) {
			...
			vObj = CreateObject("component","IBOSE.login.Validation");
			Lresult = StructNew();
			Lresult = vObj.checkAccountLockout(CheckedUserOR, theusername,thepassword,"invalidusernamepassword");
			return Lresult;
		}
	...
	public string function setSessionVars(required query CheckedUser) {
		...
		for(cntr=1;cntr<=CheckedUser.recordcount;cntr++) {
			...
			session.maxdrivesize = CheckedUser[ "MAXDRIVESIZE" ][ cntr ];
			...
			thesql = "SELECT GUID,
					   ...
					   //GUID is required to be unique and exist
			...
			for(cntg=1;cntg<=qryGMFPEOPLE.recordcount;cntg++) {
				...
				client.companycode = qryGMFPEOPLE[ "COMPANYCODE" ][ cntg ];
			...
			session.chapa = "NoPID";
			session.mytheme = "ext-all";
			session.mymessage = "";
			session.defaulttheme =  "NoDefaultTheme";
			session.avatar = "noicon.png";
	...
	public void function setupCompanySettings(required string companycode) {
		...
		thesqlA = "    SELECT COMPANYCODE,
			...
			for(cntCS=1;cntCS<=qryCOMPSETTINGS.recordcount;cntCS++) {
				...
				if (trim(qryCOMPSETTINGS[ "MAILSERVER" ][ cntCS ]) != "") session.ms = Encrypt(qryCOMPSETTINGS[ "MAILSERVER" ][ cntCS ],"mailmng");
				else session.ms = "";
				if (trim(qryCOMPSETTINGS[ "EMAILUSERNAME" ][ cntCS ]) != "") session.mu = Encrypt(qryCOMPSETTINGS[ "EMAILUSERNAME" ][ cntCS ],"mailmng");
				else session.mu = "";
				if (trim(qryCOMPSETTINGS[ "EMAILPASSWORD" ][ cntCS ]) != "") session.mp = Encrypt(qryCOMPSETTINGS[ "EMAILPASSWORD" ][ cntCS ],"mailmng");
				else session.mp = "";
				session.mt = qryCOMPSETTINGS[ "EMAILTIMEOUT" ][ cntCS ];
				session.mpt = qryCOMPSETTINGS[ "EMAILPORT" ][ cntCS ];
				session.mtls = qryCOMPSETTINGS[ "EMAILUSETLS" ][ cntCS ];
				session.mssl = qryCOMPSETTINGS[ "EMAILUSESSL" ][ cntCS ];
			}
