...
displayname="Validation"
{
	...
	public struct function checkAccountLockout(required query lockoutQry, required string theusername, required string thepassword, required string dtype) {

		retStruct = StructNew();
		if (lockoutQry.recordcount == 0) {
			retStruct["duration"] = 5;
			retStruct["message"] = "invalidunameorpassword";
			retStruct["description"] = "Invalid username";
			return retStruct;
		}
		...
		if(maxThreshhold <= failAttemptCnt + 1) {
			...
			if(totalDurationSec < 1) {
				...
				qs.addParam(name="cdate",value="#CreateODBCDateTime(now())#",cfsqltype="cf_sql_timestamp");
		...
		} else if (dtype == "validusernamepassword") {
			// Account lockout threshhold was not yet reached AND is valid username,password
			qs = new query();
			qs.addParam(name="uname",value="#ucase(theusername)#",cfsqltype="cf_sql_varchar");
			qs.addParam(name="threshholdval",value="0",cfsqltype="cf_sql_integer");
			UsqlInsert = "UPDATE EGRGUSERMASTER
    		...
    		qs.setName("updatep");
			qs.execute(sql=UsqlInsert);
			retStruct["message"] = "notincluded";
			retStruct["description"] = "notincluded";
		} else {
