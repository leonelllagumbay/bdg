...
<cfscript>
		...
		if(Isdefined("form.SIBLINGFULLNAME")) {
			for(a=1; a<=ListLen(form.SIBLINGFULLNAME,",",true); a++) {
				if(trim(ListGetAt(form.SIBLINGFULLNAME,a,",",true)) != "" && ucase(trim(ListGetAt(form.SIBLINGFULLNAME,a,",",true))) != "NA" && ucase(trim(ListGetAt(form.SIBLINGFULLNAME,a,",",true))) != "N/A") {
					onlineAppRef.insertIntoEgin21familybkgrnd(guid,applicantnumber,"SIBLING","BRO OR SIS",a);
				}
			}
		}
