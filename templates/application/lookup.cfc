...
<cfset compcode = trim(inputargs.companycode) />
<cfif trim(compcode) eq "" >
	<cfset compcode = trim(client.company_dsn) >
</cfif>


	<cfquery name="qryLookup" datasource="#compcode#_CBOSE">
    ...
    <cfquery name="countAll" datasource="#compcode#_CBOSE" >
        SELECT POSITIONCODE
		  FROM CLKPOSITION
		<cfif query NEQ "" >
		 WHERE DESCRIPTION LIKE '%#trim(query)#%'
