component  displayname="OnlineApplication" hint="Component for online applicants registration"
{
	public string function insertIntoEgmfap(required string guid, required string applicantnumber)
	{
		if(trim(form.SOURCEEMPLOYMENT) == "companysite") {
			...
			thesource = "Online: " & form.WEBSITE;
		} else if(trim(form.SOURCEEMPLOYMENT) == "walkin") {
			thesource = "Walk-in";
		} else if(trim(form.SOURCEEMPLOYMENT) == "jobfair") {
			thesource = "Job Fair: " & form.SOURCEJOBFAIRWHERE & " " & form.SOURCEJOBFAIRDATE;
		} else if(trim(form.SOURCEEMPLOYMENT) == "referral") {
			thesource = "Referred by: " & form.SOURCEREFERREDBY;
		} else {
			thesource = "Other: " & form.SOURCEOTHERVALUE; //other
		}
		...
		Egmfap.addParam(name="PHILHEALTHNUMBER",value='#Replace(trim(form.PHILHEALTHNUMBER),',','-', 'all')#',cfsqltype='cf_sql_varchar');
		...
		Egmfap.setSql("
			INSERT INTO EGMFAP ( GUID,APPLICANTNUMBER,YESREVERT,COMPANYCODE,SUFFIX,LASTNAME,FIRSTNAME,MIDDLENAME,APPLICATIONDATE,PAGIBIGNUMBER,REFERREDBY,SSSNUMBER,PHILHEALTHNUMBER,EXPECTEDSALARY,TIN,EMAILADD,POSITIONCODE,POSITIONSECONDPRIORITY,POSITIONTHIRDPRIORITY,SOURCE,RESERVED,WORKEXPRATING,RECDATECREATED,DATELASTUPDATE,POSTGRADSCHOOL,POSTGRADCOURSE,POSTGRADISGRAD,COLLEGESCHOOL,COLLEGECOURSE,COLLEGEISGRAD,VOCATIONALSCHOOL,VOCATIONALCOURSE,VOCATIONALISGRAD,REFERENCECODE)
					    VALUES ( :guid,:applicantnumber,:YESREVERT,:COMPANYCODE,:SUFFIX,:LASTNAME,:FIRSTNAME,:MIDDLENAME,:dateapplied,:PAGIBIGNUMBER,:SOURCEREFERREDBY,:SSSNUMBER,:PHILHEALTHNUMBER,:EXPECTEDSALARY,:TINNUMBER,:EMAILADDRESS,:POSITIONFIRSTPRIORITY,:POSITIONSECONDPRIORITY,:POSITIONTHIRDPRIORITY,:thesource,:reserved,:CURRENTSALARY,:RECDATECREATED,:DATELASTUPDATE,:POSTGRADSCHOOL,:POSTGRADCOURSE,:POSTGRADISGRAD,:COLLEGESCHOOL,:COLLEGECOURSE,:COLLEGEISGRAD,:VOCATIONALSCHOOL,:VOCATIONALCOURSE,:VOCATIONALISGRAD,:REFERENCECODE);
		");
