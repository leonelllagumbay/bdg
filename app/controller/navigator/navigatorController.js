Ext.define('Form.controller.navigator.navigatorController', {
    ...
    init: function() {
		this.control({
			...
			'accordionview tool[type=help]': {
				click: this.HelpTool
			},
			'accordionview tool[action=plsearch]': {
				click: this.SearchOut
			},
			'accordionview tool[action=home]': {
				click: this.GoToHome
			},
			...
			'accordionview menu[name=myworklist] menuitem': {
				click: this.openFormToApprove
			}
		});
    
	},
	
	openFormToApprove: function(dmenu,menub) {
		var eformmain = Ext.ComponentQuery.query('eformmainview')[0];
		
		if (eformmain) {
			var eformid = dmenu.name;
			var actiontype = dmenu.actiontype;
			if (actiontype) {
				eformmain.getEl().mask("Opening...");
				Ext.ss.data.getTheFormsFromeFormid(eformid,actiontype, function(result) {
					eformmain.getEl().unmask();
					try {
						eval(result);
					} catch(err) {
						Ext.Msg.show({
			...
			if (dmenu.action) window.location.href = dmenu.action;
		}
	},
	
	HelpTool: function(t) {
		window.location.href = "./resource/pdf/Help.pdf";
	},
	...
	RefreshApp: function(btn) {
		...
		var liveboarmenu = Ext.ComponentQuery.query('menu[name=myliveboard]')[0];
		liveboarmenu.removeAll();
