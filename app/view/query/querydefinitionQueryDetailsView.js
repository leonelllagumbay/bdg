	...
	initComponent: function() {  
		...
		this.items = [{
			...
			items: [{
				...
				listeners: {
			        afterrender: function(cmp) {
			            cmp.getEl().set({
			                "title": "If data is not specified, and if autoLoad is true or an Object, this store's load method is automatically called after creation. If the value of autoLoad is an Object, this Object will be passed to the store's load method."
			            });
			        }
			    }
			},{
				...
				name: 'PROXYREADER',
				fieldLabel: 'Proxy Reader',
				maxLength: 255
			},{
				name: 'PROXYWRITER',
				fieldLabel: 'Proxy Writer',
				maxLength: 255
			},{
				name: 'PROXYURL',
				...
				name: 'STOREFIELDS',
				fieldLabel: 'Field Mapping',
				listeners: {
			        afterrender: function(cmp) {
			            cmp.getEl().set({
			                "title": "Used to map the existing field to a different name (comma-separated names). This may be used in place of specifying a model configuration. The fields should be a set of Ext.data.Field configuration objects. The store will automatically create a Ext.data.Model with these fields. In general this configuration option should only be used for simple stores like a two-field store of ComboBox."
			            });
			        }
			    }
			}]
			...
			items: [{
				...
				name: 'FORMGRIDCOMBO',
				action: 'grid',
				fieldLabel: 'Enable Form Grid Combo',
				xtype: 'combobox',
				queryMode: 'local',
				store: 'query.yesno', 
				displayField: 'yesnoname',
				...
				listeners: {
			        afterrender: function(cmp) {
			            cmp.getEl().set({
			                "title": "Use eForm to view data. Format: eformid[,width,height]"
			            });
			        }
			    }
			},/*not used for now {
				fieldLabel: 'Form Grid Position',
				action: 'grid',
				xtype: 'radiogroup',
			    columns: 1,
			    items: [{
			    	boxLabel: 'Left Grid, Right Form',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'lgrf'
			    },{
			    	boxLabel: 'Left Form, Right Grid',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'lfrg'
			    },{
			    	boxLabel: 'Top Grid, Bottom Form',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'tgbf'
			    },{
			    	boxLabel: 'Top Form, Bottom Grid',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'tfbg'
			    },{
			    	boxLabel: 'Separate Window',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'sw'
			    },{
			    	boxLabel: 'Grid only (Default)',
			    	name: 'FORMGRIDPOSITION',
				    inputValue: 'go'
			    }]
			},*/{
