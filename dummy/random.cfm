<!---<cfset newVal = "0{NUMBER}--0000{NUMBER} 000000000000000000000000{NUMBER}  00{RANDOM}-{RANDOM}-{RANDOM}" >
...
<cffunction name="generateGrid" ExtDirect="true">
	...
	<cftry>
		...
		var autoeFormGrid = Ext.create('Ext.grid.Panel',{
			...
			tbar: [{
			  	text: 'Add',
				action: 'addeform',hidden: false,
				handler: function(btn) {
					var eformGrid = btn.up('grid');
					var eformid = eformGrid.id;
					Ext.ss.activate.previewGrid(eformid, 'add', function(result) { 
						eval(result);
						eForm.getForm().load({
							params: {
								eformid: eformid
				...
				handler: function(btn) {
					...
					if(selectedRecord) {
						...
						console.log(status);
						if(status != 'S') {
						Ext.ss.activate.previewGrid(eformid, 'edit', function(result) { 
							eval(result);
							var theForm = eForm.getForm();
							theForm.setValues(selectedRecord.data);
							eForm.doLayout();
							var querythisform = eForm.query('displayfield');
							for(var cntr=0;cntr<querythisform.length;cntr++) {
								
								var dispVal = querythisform[cntr].value;
								var res = dispVal.search(/\w+(?=\.(?=\w))/);
								if(res > -1) {
									var dispName = querythisform[cntr].name;
									console.log(dispName);
									var newVal = 'abc'; //'<a href=\'abc\' target=\'_parent\'>' + dispVal + '</a>';
									theForm.setValues({
										name: dispName,
										value: newVal
									});
			  	...
			  	text: 'Delete',
				action: 'deleteeform',hidden: false,
				handler: function(btn) {
			  	...
			  	text: 'Route',
				action: 'routeeform',hidden: false,
				handler: function(btn) {
			  	...
			  	text: 'View Status',
				action: 'viewpathroute',hidden: false,
				handler: function(btn) {
					var thisgrid = btn.up('grid');
					...
					if (selection) {
						...
						if(status != 'N' && status != '') {
							Ext.ss.active.generateMap(eformid,processid,pidno, function(result) {
								try {
									eval(result);
								} catch(err) { 
			  	...
			  	text: 'Open',
				action: 'openformnow',hidden: false,
				handler: function(btn) {
					var eformGrid = btn.up('grid');
					var selectedRecord = eformGrid.getSelectionModel().getSelection()[0];
					if(selectedRecord) {
			  	...
			  	text: 'Approve',
				action: 'approveformnow',hidden: false,
				handler: function(btn) {
					alert('approve');
				}
			  },{
			  	text: 'Disapprove',
				action: 'disapproveformnow',hidden: false,
				handler: function(btn) {
					alert('disapprove');
				}
			  }],
...
<cffunction name="generateForm" ExtDirect="true"> 
		...
		<cfloop array="#formTableData#" index="tableIndex" >
			...
			<cfloop array="#tableColumnData#" index="columnIndex" >
				...
				<cfelseif XTYPE EQ "filefield"> <!---filefield--->
					...
					<cfset groupItems ="{
											xtype: 'filefield',value: '#INPUTVALUE#',fieldLabel: '#FIELDLABEL#', 
								            name: 'file#fileCount#',
								            #XPOSITION#
								            #YPOSITION#
								            #ANCHORPOSITION#
	...
	<cfset auxScript = "alias: 'widget.eFormForm',
					    ...
					    buttons: [{
					    	...
					    	action: 'returntosender',hidden: false,
					    	handler: function(thiss) {
					    		var theForm = thiss.up('form').getForm();
					    		if(theForm.isValid()) {
					    			theForm.submit({
					    	...
					    	action: 'returntooriginator',hidden: false,
					    	handler: function(thiss) {
					    		var theForm = thiss.up('form').getForm();
					    		if(theForm.isValid()) {
					    			theForm.submit({
					...
					var commentStore = Ext.create('Ext.data.Store', {
								...
								listeners: {
									beforeload: function(thiss,operation,eopts) {
										var formVal  = eForm.getForm().getValues();
										thiss.proxy.extraParams.personnelidno = formVal.#firstlevel#__#firsttable#__PERSONNELIDNO; 
										thiss.proxy.extraParams.processid     = formVal.#firstlevel#__#firsttable#__PROCESSID; 
									}
								},
								proxy: {
											type: 'direct',
				...
				var formWindow = Ext.create('Ext.window.Window', { 
					    ...
					    items: eForm,
					    bbar: [{
					    	xtype: 'textfield',fieldLabel: 'Comments',hidden: false,
					    	name: 'commentsxxx',
					    	value: '',
					    	width: '40%',
					    	maxlength: 250,
							...
							xtype: 'combobox',name: 'commentslist',hidden: false,
							store: commentStore, 
							typeAhead: false,
							minChars: 1,
							pageSize: 20,
							...
							listConfig: {
				                ...
				                emptyText: 'No matching comments found.',
								// Custom rendering template for each item
				                getInnerTpl: function() {
				                    return '<h3>{title} <span style=\'float: right; font-size: .7em;\'>{lastPost}</span><br/></h3>{excerpt}';
				                }
				            },
					  }]
					}).show();" > 
