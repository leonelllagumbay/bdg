...
ExtDirect=true
{
	...
	Usql = "SELECT clkposition.POSITIONCODE AS 'Position Code', clkposition.DESCRIPTION AS 'Description', clkposition.BUDGETEDHEADCOUNT AS 'Budgeted Head Count', clkposition.TOOLTIP AS 'Org Chart Tooltip', clkposition.JOBSTYLE AS 'Org Chart Style', clkdepartment.DESCRIPTION AS 'Department', clkjobgrade.DESCRIPTION AS 'Job Grade', clkjobclasscode.DESCRIPTION AS 'Job Class', clklabortype.DESCRIPTION AS 'Labor Type', clkposition.DEPARTMENTCODE AS 'Department Code', clkposition.JOBCLASSCODE AS 'Job Class Code', clkposition.JOBGRADECODE AS 'Job Grade Code', clkposition.LABORTYPE AS 'Labor Type Code', clkposition.RECDATECREATED AS 'Date Created', clkposition.DATELASTUPDATE AS 'Date Last Update', clkposition.RECCREATEDBY AS 'Created By'
			 FROM clkposition  clkposition LEFT OUTER JOIN clkdepartment  clkdepartment ON clkposition.DEPARTMENTCODE = clkdepartment.DEPARTMENTCODE LEFT OUTER JOIN clkjobgrade  clkjobgrade ON clkposition.JOBGRADECODE = clkjobgrade.JOBGRADECODE LEFT OUTER JOIN clkjobclasscode  clkjobclasscode ON clkposition.JOBCLASSCODE = clkjobclasscode.JOBCLASSCODE LEFT OUTER JOIN clklabortype  clklabortype ON clkposition.LABORTYPE = clklabortype.LABORTYPECODE
			 #WHERE#
...
public struct function CrudSubmit() ExtDirect=true ExtFormHandler=true {
	var retVal = StructNew();
	var jobaction = trim(form.actiontype);
	if (jobaction == "ADD" || jobaction == "COPY") {
		queryService = new query();
		queryService.addParam(name="POSITIONCODE",value="#form['clkposition-POSITIONCODE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="DESCRIPTION",value="#form['clkposition-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");

		if(form['clkposition-BUDGETEDHEADCOUNT'] == "" ) {
			queryService.addParam(name="BUDGETEDHEADCOUNT",value="1",cfsqltype="CF_SQL_INTEGER");
		} else {
			queryService.addParam(name="BUDGETEDHEADCOUNT",value="#form['clkposition-BUDGETEDHEADCOUNT']#",cfsqltype="CF_SQL_INTEGER");
		}

		queryService.addParam(name="TOOLTIP",value="#form['clkposition-TOOLTIP']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="JOBSTYLE",value="#form['clkposition-JOBSTYLE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="DEPARTMENTCODE",value="#form['clkposition-DEPARTMENTCODE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="JOBCLASSCODE",value="#form['clkposition-JOBCLASSCODE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="JOBGRADECODE",value="#form['clkposition-JOBGRADECODE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="LABORTYPE",value="#form['clkposition-LABORTYPE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="RECDATECREATED",value="#CreateODBCDateTime(now())#",cfsqltype="CF_SQL_TIMESTAMP");
		queryService.addParam(name="DATELASTUPDATE",value="#CreateODBCDateTime(now())#",cfsqltype="CF_SQL_TIMESTAMP");
		queryService.addParam(name="RECCREATEDBY",value="#form['clkposition-RECCREATEDBY']#",cfsqltype="CF_SQL_VARCHAR");

		Usql = 'INSERT INTO clkposition (POSITIONCODE,DESCRIPTION,BUDGETEDHEADCOUNT,TOOLTIP,JOBSTYLE,DEPARTMENTCODE,JOBCLASSCODE,JOBGRADECODE,LABORTYPE,RECDATECREATED,DATELASTUPDATE,RECCREATEDBY)
		             VALUES (:POSITIONCODE, :DESCRIPTION, :BUDGETEDHEADCOUNT, :TOOLTIP, :JOBSTYLE, :DEPARTMENTCODE, :JOBCLASSCODE, :JOBGRADECODE, :LABORTYPE, :RECDATECREATED, :DATELASTUPDATE, :RECCREATEDBY)';
		queryService.setDatasource(session.company_dsn);
		...
		Usql = 'UPDATE clkposition
				   SET DESCRIPTION = :DESCRIPTION,BUDGETEDHEADCOUNT = :BUDGETEDHEADCOUNT,TOOLTIP = :TOOLTIP,JOBSTYLE = :JOBSTYLE,DEPARTMENTCODE = :DEPARTMENTCODE,JOBCLASSCODE = :JOBCLASSCODE,JOBGRADECODE = :JOBGRADECODE,LABORTYPE = :LABORTYPE,DATELASTUPDATE = :DATELASTUPDATE,RECCREATEDBY = :RECCREATEDBY
		...
		throw(message="Custom Error. No Action Specified",detail="Custom Error.");
	}
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM clkposition
					  WHERE POSITIONCODE = :POSITIONCODE AND DESCRIPTION = :DESCRIPTION AND BUDGETEDHEADCOUNT = :BUDGETEDHEADCOUNT AND TOOLTIP = :TOOLTIP AND JOBSTYLE = :JOBSTYLE AND DEPARTMENTCODE = :DEPARTMENTCODE AND JOBCLASSCODE = :JOBCLASSCODE AND JOBGRADECODE = :JOBGRADECODE AND LABORTYPE = :LABORTYPE AND RECCREATEDBY = :RECCREATEDBY';
		queryService.setDatasource(session.company_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
