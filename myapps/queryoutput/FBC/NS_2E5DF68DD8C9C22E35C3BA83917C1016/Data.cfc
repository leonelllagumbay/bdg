...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21empextra.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21empextra.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21empextra.ORGANIZATION AS 'Organization', cin21empextra.HIGHESTPOSITION AS 'Highest Position', cin21empextra.PERIODCOVERED AS 'Period Covered'
			 FROM #session.companycode#_cbose.cin21empextra cin21empextra
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21empextra-ORGANIZATION"] = dquery["ORGANIZATION"][b];
    			tmpresult["cin21empextra-HIGHESTPOSITION"] = dquery["HIGHEST POSITION"][b];
    			tmpresult["cin21empextra-PERIODCOVERED"] = dquery["PERIOD COVERED"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="ORGANIZATION",value="#args[b]['cin21empextra-ORGANIZATION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="HIGHESTPOSITION",value="#args[b]['cin21empextra-HIGHESTPOSITION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="PERIODCOVERED",value="#args[b]['cin21empextra-PERIODCOVERED']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21empextra (ORGANIZATION,HIGHESTPOSITION,PERIODCOVERED)
		          VALUES (:ORGANIZATION, :HIGHESTPOSITION, :PERIODCOVERED)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21empextra
						SET ORGANIZATION = :ORGANIZATION,HIGHESTPOSITION = :HIGHESTPOSITION,PERIODCOVERED = :PERIODCOVERED
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21empextra
					  WHERE ORGANIZATION = :ORGANIZATION AND HIGHESTPOSITION = :HIGHESTPOSITION AND PERIODCOVERED = :PERIODCOVERED';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21empextra.ORGANIZATION AS 'Organization', cin21empextra.HIGHESTPOSITION AS 'Highest Position', cin21empextra.PERIODCOVERED AS 'Period Covered'
			 FROM #session.companycode#_cbose.cin21empextra cin21empextra
			 WHERE  cin21empextra.PERSONNELIDNO = '#session.chapa#'
