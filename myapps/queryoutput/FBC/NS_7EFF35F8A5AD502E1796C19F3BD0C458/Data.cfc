...
ExtDirect=true
{
	...
	Usql = "SELECT eginpmsemp.PMSEMPCODE AS 'PMS Employee Code', eginpmsemp.PERSONNELIDNO AS 'Personnel Id No', eginpmsemp.RATERCODEFK AS 'Rater Code Fk', eginpmsemp.RECDATECREATED AS 'Date', cmfpa.FIRSTNAME AS 'First Name', cmfpa.LASTNAME AS 'Last Name', clkposition.POSITIONCODE AS 'Position Code', clkposition.DESCRIPTION AS 'Position', cmfpa.PERSONNELIDNO AS 'cmfpa PID', clkdepartment.DESCRIPTION AS 'Department', clkdivision.DESCRIPTION AS 'Division', cmfpa.SUPERIORCODE AS 'Immediate Superior'
			 FROM ibose_global.eginpmsemp  eginpmsemp LEFT OUTER JOIN fbc_cbose.cmfpa  cmfpa ON eginpmsemp.PERSONNELIDNO = cmfpa.PERSONNELIDNO LEFT OUTER JOIN fbc_cbose.clkposition  clkposition ON cmfpa.POSITIONCODE = clkposition.POSITIONCODE LEFT OUTER JOIN fbc_cbose.clkdepartment  clkdepartment ON cmfpa.DEPARTMENTCODE = clkdepartment.DEPARTMENTCODE LEFT OUTER JOIN fbc_cbose.clkdivision  clkdivision ON cmfpa.DIVISIONCODE = clkdivision.DIVISIONCODE
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmsemp-PMSEMPCODE"] = dquery["PMS EMPLOYEE CODE"][b];
    			tmpresult["eginpmsemp-PERSONNELIDNO"] = dquery["PERSONNEL ID NO"][b];
    			tmpresult["eginpmsemp-RATERCODEFK"] = dquery["RATER CODE FK"][b];
    			tmpresult["eginpmsemp-RECDATECREATED"] = dquery["DATE"][b];
    			...
    			tmpresult["cmfpa-PERSONNELIDNO"] = dquery["CMFPA PID"][b];
    			tmpresult["clkdepartment-DESCRIPTION"] = dquery["DEPARTMENT"][b];
    			tmpresult["clkdivision-DESCRIPTION"] = dquery["DIVISION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="PMSEMPCODE",value="#args[b]['eginpmsemp-PMSEMPCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="PERSONNELIDNO",value="#args[b]['eginpmsemp-PERSONNELIDNO']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RATERCODEFK",value="#args[b]['eginpmsemp-RATERCODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RECDATECREATED",value="#rereplace(args[b]['eginpmsemp-RECDATECREATED'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
				
		Usql = 'INSERT INTO ibose_global.eginpmsemp (PMSEMPCODE,PERSONNELIDNO,RATERCODEFK,RECDATECREATED)
		          VALUES (:PMSEMPCODE, :PERSONNELIDNO, :RATERCODEFK, :RECDATECREATED)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.eginpmsemp
						SET PMSEMPCODE = :PMSEMPCODE,PERSONNELIDNO = :PERSONNELIDNO,RATERCODEFK = :RATERCODEFK,RECDATECREATED = :RECDATECREATED
			          WHERE PMSEMPCODE = :PMSEMPCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpmsemp
					  WHERE PMSEMPCODE = :PMSEMPCODE AND PERSONNELIDNO = :PERSONNELIDNO AND RATERCODEFK = :RATERCODEFK AND RECDATECREATED = :RECDATECREATED';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
