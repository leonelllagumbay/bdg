...
ExtDirect=true
{
	...
	Usql = "SELECT egintestquery.S1 AS 'Title', egintestquery.S2 AS 'Link', egintestquery.S3 AS 'Guid', egintestquery.S4 AS 'Category', egintestquery.S5 AS 'Publish date', egintestquery.S6 AS 'Description'
			 FROM ibose_global.egintestquery egintestquery
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["egintestquery-S1"] = dquery["TITLE"][b];
    			tmpresult["egintestquery-S2"] = dquery["LINK"][b];
    			tmpresult["egintestquery-S3"] = dquery["GUID"][b];
    			tmpresult["egintestquery-S4"] = dquery["CATEGORY"][b];
    			tmpresult["egintestquery-S5"] = dquery["PUBLISH DATE"][b];
    			tmpresult["egintestquery-S6"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="S1",value="#args[b]['egintestquery-S1']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="S2",value="#args[b]['egintestquery-S2']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="S3",value="#args[b]['egintestquery-S3']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="S4",value="#args[b]['egintestquery-S4']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="S5",value="#args[b]['egintestquery-S5']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="S6",value="#args[b]['egintestquery-S6']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.egintestquery (S1,S2,S3,S4,S5,S6)
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.egintestquery
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.egintestquery
					  WHERE S1 = :S1 AND S2 = :S2 AND S3 = :S3 AND S4 = :S4 AND S5 = :S5 AND S6 = :S6';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
