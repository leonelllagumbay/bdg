...
ExtDirect=true
{
	...
	Usql = "SELECT egintestquery.ASTRING AS 'A String', egintestquery.TESTID, egintestquery.ADATETIME AS 'A Date Time', egintestquery.AFLOAT AS 'A Float', egintestquery.ANUMBER AS 'A Number', egintestquery.ATEXT AS 'A Text', egintestquery.ATIME AS 'A Time'
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'INSERT INTO EGINTESTQUERY (ASTRING,TESTID,ADATETIME,AFLOAT,ANUMBER,ATEXT,ATIME)
		          VALUES (:ASTRING, :TESTID, :ADATETIME, :AFLOAT, :ANUMBER, :ATEXT, :ATIME)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE EGINTESTQUERY
						SET ASTRING = :ASTRING,TESTID = :TESTID,ADATETIME = :ADATETIME,AFLOAT = :AFLOAT,ANUMBER = :ANUMBER,ATEXT = :ATEXT,ATIME = :ATIME
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM EGINTESTQUERY
					  WHERE ASTRING = :ASTRING AND TESTID = :TESTID AND ADATETIME = :ADATETIME AND AFLOAT = :AFLOAT AND ANUMBER = :ANUMBER AND ATEXT = :ATEXT AND ATIME = :ATIME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
