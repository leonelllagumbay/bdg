...
ExtDirect=true
{
	...
	Usql = "SELECT eginpmsrater.RATERCODE AS 'Rater Code', eginpmsrater.PMSCODEFK AS 'PMS Code Fk', eginpmsrater.RATERPID AS 'Rater PID', egrgmyibose.PERSONNELIDNO AS 'My Personnel ID', egrgmyibose.USERID AS 'My User ID', gmfpeople.FIRSTNAME AS 'First Name', gmfpeople.LASTNAME AS 'Last Name'
			 FROM ibose_global.eginpmsrater  eginpmsrater LEFT JOIN ibose_global.egrgmyibose  egrgmyibose ON eginpmsrater.RATERPID = egrgmyibose.PERSONNELIDNO LEFT JOIN ibose_global.gmfpeople  gmfpeople ON egrgmyibose.USERID = gmfpeople.USERID
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmsrater-RATERCODE"] = dquery["RATER CODE"][b];
    			tmpresult["eginpmsrater-PMSCODEFK"] = dquery["PMS CODE FK"][b];
    			tmpresult["eginpmsrater-RATERPID"] = dquery["RATER PID"][b];
    			tmpresult["egrgmyibose-PERSONNELIDNO"] = dquery["MY PERSONNEL ID"][b];
    			tmpresult["egrgmyibose-USERID"] = dquery["MY USER ID"][b];
    			tmpresult["gmfpeople-FIRSTNAME"] = dquery["FIRST NAME"][b];
    			tmpresult["gmfpeople-LASTNAME"] = dquery["LAST NAME"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="RATERCODE",value="#args[b]['eginpmsrater-RATERCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="PMSCODEFK",value="#args[b]['eginpmsrater-PMSCODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RATERPID",value="#args[b]['eginpmsrater-RATERPID']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.eginpmsrater (RATERCODE,PMSCODEFK,RATERPID)
		          VALUES (:RATERCODE, :PMSCODEFK, :RATERPID)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.eginpmsrater
						SET RATERCODE = :RATERCODE,PMSCODEFK = :PMSCODEFK,RATERPID = :RATERPID
			          WHERE RATERCODE = :RATERCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpmsrater
					  WHERE RATERCODE = :RATERCODE AND PMSCODEFK = :PMSCODEFK AND RATERPID = :RATERPID';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
