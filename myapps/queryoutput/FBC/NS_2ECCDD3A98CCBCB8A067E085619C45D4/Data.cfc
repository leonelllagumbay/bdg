...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21positnapld.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21positnapld.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT clkposition.DESCRIPTION AS 'Position', clkdepartment.DESCRIPTION AS 'Department', cin21positnapld.PRIORITY AS 'Priority', cin21positnapld.RECOMMENDATION AS 'Recommendation', cin21positnapld.POSITIONCODE AS 'Position Code', cin21positnapld.DEPARTMENTCODE AS 'Department Code', cin21positnapld.LOCATIONCODE AS 'Location Code', cin21positnapld.SECTIONCODE AS 'Section Code', cin21positnapld.DIVISIONCODE AS 'Division Code'
			 FROM #session.companycode#_cbose.cin21positnapld  cin21positnapld LEFT OUTER JOIN #session.companycode#_cbose.clkposition  clkposition ON cin21positnapld.POSITIONCODE = clkposition.POSITIONCODE LEFT OUTER JOIN #session.companycode#_cbose.clkdepartment  clkdepartment ON cin21positnapld.DEPARTMENTCODE = clkdepartment.DEPARTMENTCODE
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21positnapld-PRIORITY"] = dquery["PRIORITY"][b];
    			tmpresult["cin21positnapld-RECOMMENDATION"] = dquery["RECOMMENDATION"][b];
    			tmpresult["cin21positnapld-POSITIONCODE"] = dquery["POSITION CODE"][b];
    			tmpresult["cin21positnapld-DEPARTMENTCODE"] = dquery["DEPARTMENT CODE"][b];
    			tmpresult["cin21positnapld-LOCATIONCODE"] = dquery["LOCATION CODE"][b];
    			tmpresult["cin21positnapld-SECTIONCODE"] = dquery["SECTION CODE"][b];
    			tmpresult["cin21positnapld-DIVISIONCODE"] = dquery["DIVISION CODE"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21positnapld (DESCRIPTION)
		          VALUES (:DESCRIPTION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21positnapld
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21positnapld
					  WHERE DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT clkposition.DESCRIPTION AS 'Position', clkdepartment.DESCRIPTION AS 'Department', cin21positnapld.PRIORITY AS 'Priority', cin21positnapld.RECOMMENDATION AS 'Recommendation', cin21positnapld.POSITIONCODE AS 'Position Code', cin21positnapld.DEPARTMENTCODE AS 'Department Code', cin21positnapld.LOCATIONCODE AS 'Location Code', cin21positnapld.SECTIONCODE AS 'Section Code', cin21positnapld.DIVISIONCODE AS 'Division Code'
			 FROM #session.companycode#_cbose.cin21positnapld  cin21positnapld LEFT OUTER JOIN #session.companycode#_cbose.clkposition  clkposition ON cin21positnapld.POSITIONCODE = clkposition.POSITIONCODE LEFT OUTER JOIN #session.companycode#_cbose.clkdepartment  clkdepartment ON cin21positnapld.DEPARTMENTCODE = clkdepartment.DEPARTMENTCODE
			 WHERE  cin21positnapld.PERSONNELIDNO = '#session.chapa#'
