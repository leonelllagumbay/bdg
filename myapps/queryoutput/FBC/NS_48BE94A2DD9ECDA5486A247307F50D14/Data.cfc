...
ExtDirect=true
{
	...
	Usql = "SELECT egintestquery.AFLOAT AS 'A Float', egintestquery.ANUMBER AS 'A Number', egintestquery.ASTRING AS 'A String', egintestquery.ATEXT AS 'A Text'
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'INSERT INTO EGINTESTQUERY (AFLOAT,ANUMBER,ASTRING,ATEXT)
		          VALUES (:AFLOAT, :ANUMBER, :ASTRING, :ATEXT)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE EGINTESTQUERY
						SET AFLOAT = :AFLOAT,ANUMBER = :ANUMBER,ASTRING = :ASTRING,ATEXT = :ATEXT
			          WHERE AFLOAT = :AFLOAT';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM EGINTESTQUERY
					  WHERE AFLOAT = :AFLOAT AND ANUMBER = :ANUMBER AND ASTRING = :ASTRING AND ATEXT = :ATEXT';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
