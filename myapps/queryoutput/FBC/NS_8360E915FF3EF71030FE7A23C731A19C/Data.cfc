...
ExtDirect=true
{
	...
	Usql = "SELECT clkposition.POSITIONCODE AS 'Position Code', clkposition.PARENTPOSITION AS 'Parent Position Code', clkposition.DESCRIPTION AS 'Description', clkposition.BUDGETEDHEADCOUNT AS 'Budgeted Head Count', clkposition.TOOLTIP AS 'Org Chart Tooltip', clkposition.JOBSTYLE AS 'Org Chart Style', cmfpa.FIRSTNAME AS 'First Name', cmfpa.LASTNAME AS 'Last Name', cmfpa.MIDDLENAME AS 'Middle Name'
			 FROM #session.companycode#_cbose.clkposition  clkposition LEFT OUTER JOIN #session.companycode#_cbose.cmfpa  cmfpa ON clkposition.POSITIONCODE = cmfpa.POSITIONCODE LEFT OUTER JOIN #session.companycode#_cbose.ecrgmyibose  ecrgmyibose ON cmfpa.PERSONNELIDNO = ecrgmyibose.PERSONNELIDNO
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["clkposition-PARENTPOSITION"] = dquery["PARENT POSITION CODE"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'INSERT INTO #session.companycode#_cbose.clkposition (POSITIONCODE,PARENTPOSITION,DESCRIPTION,BUDGETEDHEADCOUNT,TOOLTIP,JOBSTYLE)
		          VALUES (:POSITIONCODE, :PARENTPOSITION, :DESCRIPTION, :BUDGETEDHEADCOUNT, :TOOLTIP, :JOBSTYLE)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE #session.companycode#_cbose.clkposition
						SET POSITIONCODE = :POSITIONCODE,PARENTPOSITION = :PARENTPOSITION,DESCRIPTION = :DESCRIPTION,BUDGETEDHEADCOUNT = :BUDGETEDHEADCOUNT,TOOLTIP = :TOOLTIP,JOBSTYLE = :JOBSTYLE
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM #session.companycode#_cbose.clkposition
					  WHERE POSITIONCODE = :POSITIONCODE AND PARENTPOSITION = :PARENTPOSITION AND DESCRIPTION = :DESCRIPTION AND BUDGETEDHEADCOUNT = :BUDGETEDHEADCOUNT AND TOOLTIP = :TOOLTIP AND JOBSTYLE = :JOBSTYLE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
