...
ExtDirect=true
{
	...
	Usql = "SELECT eginpms.PMSCODE AS 'PMS Code', eginpms.PMSNAME AS 'Name', eginpms.FREQUENCY AS 'Frequency', eginpms.RECCREATEDBY AS 'Created By', eginpms.RECDATECREATED AS 'Date Record Created'
			 FROM ibose_global.eginpms eginpms
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpms-PMSCODE"] = dquery["PMS CODE"][b];
    			tmpresult["eginpms-PMSNAME"] = dquery["NAME"][b];
    			tmpresult["eginpms-FREQUENCY"] = dquery["FREQUENCY"][b];
    			tmpresult["eginpms-RECCREATEDBY"] = dquery["CREATED BY"][b];
    			tmpresult["eginpms-RECDATECREATED"] = dquery["DATE RECORD CREATED"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="PMSCODE",value="#args[b]['eginpms-PMSCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="PMSNAME",value="#args[b]['eginpms-PMSNAME']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="FREQUENCY",value="#args[b]['eginpms-FREQUENCY']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RECCREATEDBY",value="#args[b]['eginpms-RECCREATEDBY']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RECDATECREATED",value="#rereplace(args[b]['eginpms-RECDATECREATED'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
				
		Usql = 'INSERT INTO ibose_global.eginpms (PMSCODE,PMSNAME,FREQUENCY,RECCREATEDBY,RECDATECREATED)
		          VALUES (:PMSCODE, :PMSNAME, :FREQUENCY, :RECCREATEDBY, :RECDATECREATED)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.eginpms
						SET PMSCODE = :PMSCODE,PMSNAME = :PMSNAME,FREQUENCY = :FREQUENCY,RECCREATEDBY = :RECCREATEDBY,RECDATECREATED = :RECDATECREATED
			          WHERE PMSCODE = :PMSCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpms
					  WHERE PMSCODE = :PMSCODE AND PMSNAME = :PMSNAME AND FREQUENCY = :FREQUENCY AND RECCREATEDBY = :RECCREATEDBY AND RECDATECREATED = :RECDATECREATED';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
