...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21empawards.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21empawards.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21empawards.REWARDCODE AS 'Reward Code', clkmeasures.DESCRIPTION AS 'Measure Description', clkmeasures.POINTS AS 'Points', cin21empawards.DATEAWARDED AS 'Date Awarded'
			 FROM #session.companycode#_cbose.cin21empawards  cin21empawards LEFT OUTER JOIN #session.companycode#_cbose.clkmeasures  clkmeasures ON cin21empawards.MEASURECODE = clkmeasures.MEASURESCODE
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21empawards-REWARDCODE"] = dquery["REWARD CODE"][b];
    			tmpresult["clkmeasures-DESCRIPTION"] = dquery["MEASURE DESCRIPTION"][b];
    			tmpresult["clkmeasures-POINTS"] = dquery["POINTS"][b];
    			tmpresult["cin21empawards-DATEAWARDED"] = dquery["DATE AWARDED"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="REWARDCODE",value="#args[b]['cin21empawards-REWARDCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DATEAWARDED",value="#rereplace(args[b]['cin21empawards-DATEAWARDED'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
				
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21empawards (REWARDCODE,DATEAWARDED)
		          VALUES (:REWARDCODE, :DATEAWARDED)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21empawards
						SET REWARDCODE = :REWARDCODE,DATEAWARDED = :DATEAWARDED
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="REWARDCODE",value="#args[b]['cin21empawards-REWARDCODE']#",cfsqltype="CF_SQL_VARCHAR");
		queryService.addParam(name="DATEAWARDED",value="#args[b]['cin21empawards-DATEAWARDED']#",cfsqltype="CF_SQL_VARCHAR");
		
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21empawards
					  WHERE REWARDCODE = :REWARDCODE AND DATEAWARDED = :DATEAWARDED';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21empawards.REWARDCODE AS 'Reward Code', clkmeasures.DESCRIPTION AS 'Measure Description', clkmeasures.POINTS AS 'Points', cin21empawards.DATEAWARDED AS 'Date Awarded'
			 FROM #session.companycode#_cbose.cin21empawards  cin21empawards LEFT OUTER JOIN #session.companycode#_cbose.clkmeasures  clkmeasures ON cin21empawards.MEASURECODE = clkmeasures.MEASURESCODE
			 WHERE  cin21empawards.PERSONNELIDNO = '#session.chapa#'
