...
ExtDirect=true
{
	...
	Usql = "SELECT clkmeasures.MEASURESCODE AS 'Measure Code', clkmeasures.DESCRIPTION AS 'Description', clkmeasures.POINTS AS 'Points'
			 FROM #session.companycode#_cbose.clkmeasures clkmeasures
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["clkmeasures-MEASURESCODE"] = dquery["MEASURE CODE"][b];
    			tmpresult["clkmeasures-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="MEASURESCODE",value="#args[b]['clkmeasures-MEASURESCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['clkmeasures-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="POINTS",value="#args[b]['clkmeasures-POINTS']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO #session.companycode#_cbose.clkmeasures (MEASURESCODE,DESCRIPTION,POINTS)
		          VALUES (:MEASURESCODE, :DESCRIPTION, :POINTS)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE #session.companycode#_cbose.clkmeasures
						SET MEASURESCODE = :MEASURESCODE,DESCRIPTION = :DESCRIPTION,POINTS = :POINTS
			          WHERE MEASURESCODE = :MEASURESCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM #session.companycode#_cbose.clkmeasures
					  WHERE MEASURESCODE = :MEASURESCODE AND DESCRIPTION = :DESCRIPTION AND POINTS = :POINTS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
