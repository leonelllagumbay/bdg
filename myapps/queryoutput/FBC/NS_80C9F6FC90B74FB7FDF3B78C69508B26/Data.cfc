...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  eginpmscomment.TOUSERID = '#session.userid#'";
	} else {
		WHERE = "#WHERE#  AND  eginpmscomment.TOUSERID = '#session.userid#'";
	}
	...
	if (trim(ORDERBY) == "") {
		ORDERBY = "ORDER BY eginpmscomment.RECDATECREATED DESC";
	} else {
		ORDERBY = "ORDER BY #ORDERBY# ,eginpmscomment.RECDATECREATED DESC";
	}

	Usql = "SELECT eginpmscomment.COMMENTCODE AS 'Comment Code', eginpmscomment.CONTENT AS 'Content', eginpmscomment.FROMUSERID AS 'From', eginpmscomment.TOUSERID AS 'To', eginpmscomment.RECDATECREATED AS 'Date', eginpmscomment.PMSCODEFK AS 'PMS Code'
			 FROM ibose_global.eginpmscomment eginpmscomment
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmscomment-COMMENTCODE"] = dquery["COMMENT CODE"][b];
    			tmpresult["eginpmscomment-CONTENT"] = dquery["CONTENT"][b];
    			tmpresult["eginpmscomment-FROMUSERID"] = dquery["FROM"][b];
    			tmpresult["eginpmscomment-TOUSERID"] = dquery["TO"][b];
    			tmpresult["eginpmscomment-RECDATECREATED"] = dquery["DATE"][b];
    			tmpresult["eginpmscomment-PMSCODEFK"] = dquery["PMS CODE"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="CONTENT",value="#args[b]['eginpmscomment-CONTENT']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="FROMUSERID",value="#args[b]['eginpmscomment-FROMUSERID']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="TOUSERID",value="#args[b]['eginpmscomment-TOUSERID']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RECDATECREATED",value="#rereplace(args[b]['eginpmscomment-RECDATECREATED'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
				queryService.addParam(name="PMSCODEFK",value="#args[b]['eginpmscomment-PMSCODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.eginpmscomment (CONTENT,FROMUSERID,TOUSERID,RECDATECREATED,PMSCODEFK)
		          VALUES (:CONTENT, :FROMUSERID, :TOUSERID, :RECDATECREATED, :PMSCODEFK)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="COMMENTCODE",value="#args[b]['eginpmscomment-COMMENTCODE']#",cfsqltype="CF_SQL_INTEGER");
			queryService.addParam(name="CONTENT",value="#args[b]['eginpmscomment-CONTENT']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="FROMUSERID",value="#args[b]['eginpmscomment-FROMUSERID']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="TOUSERID",value="#args[b]['eginpmscomment-TOUSERID']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RECDATECREATED",value="#rereplace(args[b]['eginpmscomment-RECDATECREATED'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
		...
		Usql = 'UPDATE ibose_global.eginpmscomment
						SET COMMENTCODE = :COMMENTCODE,CONTENT = :CONTENT,FROMUSERID = :FROMUSERID,TOUSERID = :TOUSERID,RECDATECREATED = :RECDATECREATED,PMSCODEFK = :PMSCODEFK
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpmscomment
					  WHERE COMMENTCODE = :COMMENTCODE AND CONTENT = :CONTENT AND FROMUSERID = :FROMUSERID AND TOUSERID = :TOUSERID AND RECDATECREATED = :RECDATECREATED AND PMSCODEFK = :PMSCODEFK';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT eginpmscomment.COMMENTCODE AS 'Comment Code', eginpmscomment.CONTENT AS 'Content', eginpmscomment.FROMUSERID AS 'From', eginpmscomment.TOUSERID AS 'To', eginpmscomment.RECDATECREATED AS 'Date', eginpmscomment.PMSCODEFK AS 'PMS Code'
			 FROM ibose_global.eginpmscomment eginpmscomment
			 WHERE  eginpmscomment.TOUSERID = '#session.userid#'
			 
			 
			 ORDER BY eginpmscomment.RECDATECREATED DESC";
