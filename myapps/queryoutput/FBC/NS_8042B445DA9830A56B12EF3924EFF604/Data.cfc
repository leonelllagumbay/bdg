...
ExtDirect=true
{
	...
	Usql = "SELECT clkjobclasscode.JOBCLASSCODE AS 'Job Class Code', clkjobclasscode.DESCRIPTION AS 'Description'
			 FROM #session.companycode#_cbose.clkjobclasscode clkjobclasscode
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["clkjobclasscode-JOBCLASSCODE"] = dquery["JOB CLASS CODE"][b];
    			tmpresult["clkjobclasscode-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="JOBCLASSCODE",value="#args[b]['clkjobclasscode-JOBCLASSCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['clkjobclasscode-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO #session.companycode#_cbose.clkjobclasscode (JOBCLASSCODE,DESCRIPTION)
		          VALUES (:JOBCLASSCODE, :DESCRIPTION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE #session.companycode#_cbose.clkjobclasscode
						SET JOBCLASSCODE = :JOBCLASSCODE,DESCRIPTION = :DESCRIPTION
			          WHERE JOBCLASSCODE = :JOBCLASSCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM #session.companycode#_cbose.clkjobclasscode
					  WHERE JOBCLASSCODE = :JOBCLASSCODE AND DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
