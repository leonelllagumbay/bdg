...
ExtDirect=true
{
	...
	if (trim(ORDERBY) == "") {
		ORDERBY = "ORDER BY clkjobgrade.JOBGRADECODE ASC";
	} else {
		ORDERBY = "ORDER BY #ORDERBY# ,clkjobgrade.JOBGRADECODE ASC";
	}

	Usql = "SELECT clkjobgrade.JOBGRADECODE AS 'Job Grade Code', clkjobgrade.DESCRIPTION AS 'Description', clkjobgrade.JOBABBREVIATION AS 'Abbreviation'
			 FROM #session.companycode#_cbose.clkjobgrade clkjobgrade
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["clkjobgrade-JOBGRADECODE"] = dquery["JOB GRADE CODE"][b];
    			tmpresult["clkjobgrade-DESCRIPTION"] = dquery["DESCRIPTION"][b];
    			tmpresult["clkjobgrade-JOBABBREVIATION"] = dquery["ABBREVIATION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="JOBGRADECODE",value="#args[b]['clkjobgrade-JOBGRADECODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['clkjobgrade-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="JOBABBREVIATION",value="#args[b]['clkjobgrade-JOBABBREVIATION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO #session.companycode#_cbose.clkjobgrade (JOBGRADECODE,DESCRIPTION,JOBABBREVIATION)
		          VALUES (:JOBGRADECODE, :DESCRIPTION, :JOBABBREVIATION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE #session.companycode#_cbose.clkjobgrade
						SET JOBGRADECODE = :JOBGRADECODE,DESCRIPTION = :DESCRIPTION,JOBABBREVIATION = :JOBABBREVIATION
			          WHERE JOBGRADECODE = :JOBGRADECODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM #session.companycode#_cbose.clkjobgrade
					  WHERE JOBGRADECODE = :JOBGRADECODE AND DESCRIPTION = :DESCRIPTION AND JOBABBREVIATION = :JOBABBREVIATION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT clkjobgrade.JOBGRADECODE AS 'Job Grade Code', clkjobgrade.DESCRIPTION AS 'Description', clkjobgrade.JOBABBREVIATION AS 'Abbreviation'
			 ...
			 ORDER BY clkjobgrade.JOBGRADECODE ASC";
