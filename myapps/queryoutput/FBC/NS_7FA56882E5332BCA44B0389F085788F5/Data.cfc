...
ExtDirect=true
{
	...
	Usql = "SELECT eginpmsempcpitems.CPITEMCODE AS 'CP Developmental Plan Code', eginpmsempcpitems.KRACODEFK AS 'KRA Code Fk', eginpmsempcpitems.DEVELOPMENTALAREAS AS 'DEVELOPMENTAL AREAS', eginpmsempcpitems.MEASUREOFIMPROVEMENT AS 'MEASURE OF IMPROVEMENT', eginpmsempcpitems.DEVELOPMENTALACTIVITIES AS 'DEVELOPMENTAL ACTIVITIES', eginpmsempcpitems.MEASUREMENTDATE AS 'MEASUREMENT DATE', eginpmsempcpitems.COMMENTS AS 'COMMENTS', eginpmsempkra.KRACODE AS 'KRA Code'
			 FROM ibose_global.eginpmsempcpitems  eginpmsempcpitems LEFT OUTER JOIN ibose_global.eginpmsempkra  eginpmsempkra ON eginpmsempcpitems.KRACODEFK = eginpmsempkra.KRACODE
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmsempcpitems-CPITEMCODE"] = dquery["CP DEVELOPMENTAL PLAN CODE"][b];
    			tmpresult["eginpmsempcpitems-KRACODEFK"] = dquery["KRA CODE FK"][b];
    			tmpresult["eginpmsempcpitems-DEVELOPMENTALAREAS"] = dquery["DEVELOPMENTAL AREAS"][b];
    			tmpresult["eginpmsempcpitems-MEASUREOFIMPROVEMENT"] = dquery["MEASURE OF IMPROVEMENT"][b];
    			tmpresult["eginpmsempcpitems-DEVELOPMENTALACTIVITIES"] = dquery["DEVELOPMENTAL ACTIVITIES"][b];
    			tmpresult["eginpmsempcpitems-MEASUREMENTDATE"] = dquery["MEASUREMENT DATE"][b];
    			tmpresult["eginpmsempcpitems-COMMENTS"] = dquery["COMMENTS"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="KRACODEFK",value="#args[b]['eginpmsempcpitems-KRACODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DEVELOPMENTALAREAS",value="#args[b]['eginpmsempcpitems-DEVELOPMENTALAREAS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="MEASUREOFIMPROVEMENT",value="#args[b]['eginpmsempcpitems-MEASUREOFIMPROVEMENT']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DEVELOPMENTALACTIVITIES",value="#args[b]['eginpmsempcpitems-DEVELOPMENTALACTIVITIES']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="MEASUREMENTDATE",value="#rereplace(args[b]['eginpmsempcpitems-MEASUREMENTDATE'],'[a-zA-Z]',' ')#",cfsqltype="CF_SQL_TIMESTAMP");
				queryService.addParam(name="COMMENTS",value="#args[b]['eginpmsempcpitems-COMMENTS']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.eginpmsempcpitems (KRACODEFK,DEVELOPMENTALAREAS,MEASUREOFIMPROVEMENT,DEVELOPMENTALACTIVITIES,MEASUREMENTDATE,COMMENTS)
		          VALUES (:KRACODEFK, :DEVELOPMENTALAREAS, :MEASUREOFIMPROVEMENT, :DEVELOPMENTALACTIVITIES, :MEASUREMENTDATE, :COMMENTS)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="CPITEMCODE",value="#args[b]['eginpmsempcpitems-CPITEMCODE']#",cfsqltype="CF_SQL_INTEGER");
			queryService.addParam(name="KRACODEFK",value="#args[b]['eginpmsempcpitems-KRACODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DEVELOPMENTALAREAS",value="#args[b]['eginpmsempcpitems-DEVELOPMENTALAREAS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="MEASUREOFIMPROVEMENT",value="#args[b]['eginpmsempcpitems-MEASUREOFIMPROVEMENT']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DEVELOPMENTALACTIVITIES",value="#args[b]['eginpmsempcpitems-DEVELOPMENTALACTIVITIES']#",cfsqltype="CF_SQL_VARCHAR");
		...
		Usql = 'UPDATE ibose_global.eginpmsempcpitems
						SET CPITEMCODE = :CPITEMCODE,KRACODEFK = :KRACODEFK,DEVELOPMENTALAREAS = :DEVELOPMENTALAREAS,MEASUREOFIMPROVEMENT = :MEASUREOFIMPROVEMENT,DEVELOPMENTALACTIVITIES = :DEVELOPMENTALACTIVITIES,MEASUREMENTDATE = :MEASUREMENTDATE,COMMENTS = :COMMENTS
			          WHERE CPITEMCODE = :CPITEMCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpmsempcpitems
					  WHERE CPITEMCODE = :CPITEMCODE AND KRACODEFK = :KRACODEFK AND DEVELOPMENTALAREAS = :DEVELOPMENTALAREAS AND MEASUREOFIMPROVEMENT = :MEASUREOFIMPROVEMENT AND DEVELOPMENTALACTIVITIES = :DEVELOPMENTALACTIVITIES AND MEASUREMENTDATE = :MEASUREMENTDATE AND COMMENTS = :COMMENTS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
