...
ExtDirect=true
{
	...
	Usql = "SELECT glkeduclevel.EDUCLEVEL AS 'Educ Level Code', glkeduclevel.DESCRIPTION AS 'Description'
			 FROM ibose_global.glkeduclevel glkeduclevel
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["glkeduclevel-EDUCLEVEL"] = dquery["EDUC LEVEL CODE"][b];
    			tmpresult["glkeduclevel-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="EDUCLEVEL",value="#args[b]['glkeduclevel-EDUCLEVEL']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['glkeduclevel-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.glkeduclevel (EDUCLEVEL,DESCRIPTION)
		          VALUES (:EDUCLEVEL, :DESCRIPTION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.glkeduclevel
						SET EDUCLEVEL = :EDUCLEVEL,DESCRIPTION = :DESCRIPTION
			          WHERE EDUCLEVEL = :EDUCLEVEL';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.glkeduclevel
					  WHERE EDUCLEVEL = :EDUCLEVEL AND DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
