...
ExtDirect=true
{
	...
	Usql = "SELECT eginpmsempcp.CPCODE AS 'Career Planning Code', eginpmsempcp.KRACODEFK AS 'KRA Code Fk', eginpmsempcp.STRENGTHS AS 'Strengths', eginpmsempcp.CAREERPATHSHORTTERM AS 'Possible Career Path (Short Term)', eginpmsempcp.CAREERPATHMEDIUMTERM AS 'Possible Career Path (Medium Term)', eginpmsempcp.CAREERPATHLONGTERM AS 'Possible Career Path (Long Term)', eginpmsempcp.COMMENTS AS 'Comments', eginpmsempkra.KRACODE AS 'KRA Code'
			 FROM ibose_global.eginpmsempcp  eginpmsempcp LEFT OUTER JOIN ibose_global.eginpmsempkra  eginpmsempkra ON eginpmsempkra.KRACODE = eginpmsempcp.KRACODEFK
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmsempcp-CPCODE"] = dquery["CAREER PLANNING CODE"][b];
    			tmpresult["eginpmsempcp-KRACODEFK"] = dquery["KRA CODE FK"][b];
    			tmpresult["eginpmsempcp-STRENGTHS"] = dquery["STRENGTHS"][b];
    			tmpresult["eginpmsempcp-CAREERPATHSHORTTERM"] = dquery["POSSIBLE CAREER PATH (SHORT TERM)"][b];
    			tmpresult["eginpmsempcp-CAREERPATHMEDIUMTERM"] = dquery["POSSIBLE CAREER PATH (MEDIUM TERM)"][b];
    			tmpresult["eginpmsempcp-CAREERPATHLONGTERM"] = dquery["POSSIBLE CAREER PATH (LONG TERM)"][b];
    			tmpresult["eginpmsempcp-COMMENTS"] = dquery["COMMENTS"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="CPCODE",value="#args[b]['eginpmsempcp-CPCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="KRACODEFK",value="#args[b]['eginpmsempcp-KRACODEFK']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="STRENGTHS",value="#args[b]['eginpmsempcp-STRENGTHS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="CAREERPATHSHORTTERM",value="#args[b]['eginpmsempcp-CAREERPATHSHORTTERM']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="CAREERPATHMEDIUMTERM",value="#args[b]['eginpmsempcp-CAREERPATHMEDIUMTERM']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="CAREERPATHLONGTERM",value="#args[b]['eginpmsempcp-CAREERPATHLONGTERM']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="COMMENTS",value="#args[b]['eginpmsempcp-COMMENTS']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.eginpmsempcp (CPCODE,KRACODEFK,STRENGTHS,CAREERPATHSHORTTERM,CAREERPATHMEDIUMTERM,CAREERPATHLONGTERM,COMMENTS)
		          VALUES (:CPCODE, :KRACODEFK, :STRENGTHS, :CAREERPATHSHORTTERM, :CAREERPATHMEDIUMTERM, :CAREERPATHLONGTERM, :COMMENTS)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.eginpmsempcp
						SET CPCODE = :CPCODE,KRACODEFK = :KRACODEFK,STRENGTHS = :STRENGTHS,CAREERPATHSHORTTERM = :CAREERPATHSHORTTERM,CAREERPATHMEDIUMTERM = :CAREERPATHMEDIUMTERM,CAREERPATHLONGTERM = :CAREERPATHLONGTERM,COMMENTS = :COMMENTS
			          WHERE CPCODE = :CPCODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eginpmsempcp
					  WHERE CPCODE = :CPCODE AND KRACODEFK = :KRACODEFK AND STRENGTHS = :STRENGTHS AND CAREERPATHSHORTTERM = :CAREERPATHSHORTTERM AND CAREERPATHMEDIUMTERM = :CAREERPATHMEDIUMTERM AND CAREERPATHLONGTERM = :CAREERPATHLONGTERM AND COMMENTS = :COMMENTS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
