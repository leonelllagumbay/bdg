...
ExtDirect=true
{
	...
	Usql = "SELECT glkschool.SCHOOLCODE AS 'School Code', glkschool.SCHOOLNAME AS 'School Name'
			 FROM ibose_global.glkschool glkschool
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["glkschool-SCHOOLCODE"] = dquery["SCHOOL CODE"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="SCHOOLCODE",value="#args[b]['glkschool-SCHOOLCODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="SCHOOLNAME",value="#args[b]['glkschool-SCHOOLNAME']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.glkschool (SCHOOLCODE,SCHOOLNAME)
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.glkschool
						SET SCHOOLCODE = :SCHOOLCODE,SCHOOLNAME = :SCHOOLNAME
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.glkschool
					  WHERE SCHOOLCODE = :SCHOOLCODE AND SCHOOLNAME = :SCHOOLNAME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
