...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21actualskills.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21actualskills.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21actualskills.LEVELOFADEPTNESS AS 'Level of Adeptness', clkskill.DESCRIPTION AS 'Skill'
			 FROM #session.companycode#_cbose.cin21actualskills  cin21actualskills LEFT OUTER JOIN #session.companycode#_cbose.clkskill  clkskill ON cin21actualskills.SKILLCODE = clkskill.SKILLCODE
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21actualskills-LEVELOFADEPTNESS"] = dquery["LEVEL OF ADEPTNESS"][b];
    			tmpresult["clkskill-DESCRIPTION"] = dquery["SKILL"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="LEVELOFADEPTNESS",value="#args[b]['cin21actualskills-LEVELOFADEPTNESS']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21actualskills (LEVELOFADEPTNESS)
		          VALUES (:LEVELOFADEPTNESS)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21actualskills
						SET LEVELOFADEPTNESS = :LEVELOFADEPTNESS
			          WHERE LEVELOFADEPTNESS = :LEVELOFADEPTNESS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21actualskills
					  WHERE LEVELOFADEPTNESS = :LEVELOFADEPTNESS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21actualskills.LEVELOFADEPTNESS AS 'Level of Adeptness', clkskill.DESCRIPTION AS 'Skill'
			 FROM #session.companycode#_cbose.cin21actualskills  cin21actualskills LEFT OUTER JOIN #session.companycode#_cbose.clkskill  clkskill ON cin21actualskills.SKILLCODE = clkskill.SKILLCODE
			 WHERE  cin21actualskills.PERSONNELIDNO = '#session.chapa#'
