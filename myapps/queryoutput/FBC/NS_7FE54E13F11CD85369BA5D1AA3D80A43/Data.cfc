...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  eginpmscomment.TOUSERID = 'ALL'";
	} else {
		WHERE = "#WHERE#  AND  eginpmscomment.TOUSERID = 'ALL'";
	}
	...
	Usql = "SELECT eginpmscomment.COMMENTCODE AS 'Comment Code', eginpmscomment.CONTENT AS 'Content', eginpmscomment.FROMUSERID AS 'From', eginpmscomment.TOUSERID AS 'To', eginpmscomment.RECDATECREATED AS 'Date', eginpmscomment.PMSCODEFK AS 'PMS Code Fk'
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eginpmscomment-PMSCODEFK"] = dquery["PMS CODE FK"][b];
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT eginpmscomment.COMMENTCODE AS 'Comment Code', eginpmscomment.CONTENT AS 'Content', eginpmscomment.FROMUSERID AS 'From', eginpmscomment.TOUSERID AS 'To', eginpmscomment.RECDATECREATED AS 'Date', eginpmscomment.PMSCODEFK AS 'PMS Code Fk'
			 FROM ibose_global.eginpmscomment eginpmscomment
			 WHERE  eginpmscomment.TOUSERID = 'ALL'
