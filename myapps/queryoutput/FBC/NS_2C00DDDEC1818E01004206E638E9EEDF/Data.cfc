...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21achievements.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21achievements.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21achievements.AWARDTITLE AS 'Award Title', cin21achievements.NATURECATEGORY AS 'Nature or Category', cin21achievements.DATEGIVEN AS 'Date Given', cin21achievements.GIVENBY AS 'Given By'
			 FROM #session.companycode#_cbose.cin21achievements cin21achievements
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21achievements-AWARDTITLE"] = dquery["AWARD TITLE"][b];
    			tmpresult["cin21achievements-NATURECATEGORY"] = dquery["NATURE OR CATEGORY"][b];
    			tmpresult["cin21achievements-DATEGIVEN"] = dquery["DATE GIVEN"][b];
    			tmpresult["cin21achievements-GIVENBY"] = dquery["GIVEN BY"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="AWARDTITLE",value="#args[b]['cin21achievements-AWARDTITLE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="NATURECATEGORY",value="#args[b]['cin21achievements-NATURECATEGORY']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DATEGIVEN",value="#args[b]['cin21achievements-DATEGIVEN']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="GIVENBY",value="#args[b]['cin21achievements-GIVENBY']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21achievements (AWARDTITLE,NATURECATEGORY,DATEGIVEN,GIVENBY)
		          VALUES (:AWARDTITLE, :NATURECATEGORY, :DATEGIVEN, :GIVENBY)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21achievements
						SET AWARDTITLE = :AWARDTITLE,NATURECATEGORY = :NATURECATEGORY,DATEGIVEN = :DATEGIVEN,GIVENBY = :GIVENBY
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21achievements
					  WHERE AWARDTITLE = :AWARDTITLE AND NATURECATEGORY = :NATURECATEGORY AND DATEGIVEN = :DATEGIVEN AND GIVENBY = :GIVENBY';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21achievements.AWARDTITLE AS 'Award Title', cin21achievements.NATURECATEGORY AS 'Nature or Category', cin21achievements.DATEGIVEN AS 'Date Given', cin21achievements.GIVENBY AS 'Given By'
			 FROM #session.companycode#_cbose.cin21achievements cin21achievements
			 WHERE  cin21achievements.PERSONNELIDNO = '#session.chapa#'
