...
ExtDirect=true
{
	...
	Usql = "SELECT clklabortype.LABORTYPECODE AS 'Labor Type Code', clklabortype.DESCRIPTION AS 'Description'
			 FROM #session.companycode#_cbose.clklabortype clklabortype
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["clklabortype-LABORTYPECODE"] = dquery["LABOR TYPE CODE"][b];
    			tmpresult["clklabortype-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="LABORTYPECODE",value="#args[b]['clklabortype-LABORTYPECODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['clklabortype-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO #session.companycode#_cbose.clklabortype (LABORTYPECODE,DESCRIPTION)
		          VALUES (:LABORTYPECODE, :DESCRIPTION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE #session.companycode#_cbose.clklabortype
						SET LABORTYPECODE = :LABORTYPECODE,DESCRIPTION = :DESCRIPTION
			          WHERE LABORTYPECODE = :LABORTYPECODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM #session.companycode#_cbose.clklabortype
					  WHERE LABORTYPECODE = :LABORTYPECODE AND DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
