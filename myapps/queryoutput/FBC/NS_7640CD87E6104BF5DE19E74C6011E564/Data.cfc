...
ExtDirect=true
{
	...
	Usql = "SELECT cbr201.CASUALMONTHS AS 'Y'
			 FROM fbc_cbose.cbr201 cbr201
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cbr201-CASUALMONTHS"] = dquery["Y"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="CASUALMONTHS",value="#args[b]['cbr201-CASUALMONTHS']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO fbc_cbose.cbr201 (CASUALMONTHS)
		          VALUES (:CASUALMONTHS)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE fbc_cbose.cbr201
						SET CASUALMONTHS = :CASUALMONTHS
			          WHERE CASUALMONTHS = :CASUALMONTHS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM fbc_cbose.cbr201
					  WHERE CASUALMONTHS = :CASUALMONTHS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
