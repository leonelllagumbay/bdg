...
ExtDirect=true
{
	...
	Usql = "SELECT glkdegree.DEGREECODE AS 'Degree Code', glkdegree.DESCRIPTION AS 'Description'
			 FROM ibose_global.glkdegree glkdegree
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["glkdegree-DEGREECODE"] = dquery["DEGREE CODE"][b];
    			tmpresult["glkdegree-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="DEGREECODE",value="#args[b]['glkdegree-DEGREECODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['glkdegree-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.glkdegree (DEGREECODE,DESCRIPTION)
		          VALUES (:DEGREECODE, :DESCRIPTION)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.glkdegree
						SET DEGREECODE = :DEGREECODE,DESCRIPTION = :DESCRIPTION
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.glkdegree
					  WHERE DEGREECODE = :DEGREECODE AND DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
