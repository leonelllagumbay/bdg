...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21relative.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21relative.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21relative.NAME AS 'Name', cin21relative.RELATIONSHIP AS 'Relationship', cin21relative.COMPANY AS 'Company', cin21relative.POSITION AS 'Position', cin21relative.DEPARTMENTAREAD AS 'Department Area'
			 FROM #session.companycode#_cbose.cin21relative cin21relative
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21relative-NAME"] = dquery["NAME"][b];
    			tmpresult["cin21relative-RELATIONSHIP"] = dquery["RELATIONSHIP"][b];
    			tmpresult["cin21relative-COMPANY"] = dquery["COMPANY"][b];
    			tmpresult["cin21relative-POSITION"] = dquery["POSITION"][b];
    			tmpresult["cin21relative-DEPARTMENTAREAD"] = dquery["DEPARTMENT AREA"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="NAME",value="#args[b]['cin21relative-NAME']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RELATIONSHIP",value="#args[b]['cin21relative-RELATIONSHIP']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="COMPANY",value="#args[b]['cin21relative-COMPANY']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="POSITION",value="#args[b]['cin21relative-POSITION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DEPARTMENTAREAD",value="#args[b]['cin21relative-DEPARTMENTAREAD']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21relative (NAME,RELATIONSHIP,COMPANY,POSITION,DEPARTMENTAREAD)
		          VALUES (:NAME, :RELATIONSHIP, :COMPANY, :POSITION, :DEPARTMENTAREAD)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21relative
						SET NAME = :NAME,RELATIONSHIP = :RELATIONSHIP,COMPANY = :COMPANY,POSITION = :POSITION,DEPARTMENTAREAD = :DEPARTMENTAREAD
			          WHERE NAME = :NAME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21relative
					  WHERE NAME = :NAME AND RELATIONSHIP = :RELATIONSHIP AND COMPANY = :COMPANY AND POSITION = :POSITION AND DEPARTMENTAREAD = :DEPARTMENTAREAD';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21relative.NAME AS 'Name', cin21relative.RELATIONSHIP AS 'Relationship', cin21relative.COMPANY AS 'Company', cin21relative.POSITION AS 'Position', cin21relative.DEPARTMENTAREAD AS 'Department Area'
			 FROM #session.companycode#_cbose.cin21relative cin21relative
			 WHERE  cin21relative.PERSONNELIDNO = '#session.chapa#'
