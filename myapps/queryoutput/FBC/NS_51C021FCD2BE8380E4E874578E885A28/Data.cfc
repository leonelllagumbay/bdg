...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21miscinfo1.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21miscinfo1.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21miscinfo1.SPECIALTALENTS AS 'Special Talents', cin21miscinfo1.CLASSIFICATION AS 'Years of Experience', cin21miscinfo1.RANKHELD AS 'Level of Expertise'
			 FROM #session.companycode#_cbose.cin21miscinfo1 cin21miscinfo1
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21miscinfo1-SPECIALTALENTS"] = dquery["SPECIAL TALENTS"][b];
    			tmpresult["cin21miscinfo1-CLASSIFICATION"] = dquery["YEARS OF EXPERIENCE"][b];
    			tmpresult["cin21miscinfo1-RANKHELD"] = dquery["LEVEL OF EXPERTISE"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="SPECIALTALENTS",value="#args[b]['cin21miscinfo1-SPECIALTALENTS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="CLASSIFICATION",value="#args[b]['cin21miscinfo1-CLASSIFICATION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="RANKHELD",value="#args[b]['cin21miscinfo1-RANKHELD']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21miscinfo1 (SPECIALTALENTS,CLASSIFICATION,RANKHELD)
		          VALUES (:SPECIALTALENTS, :CLASSIFICATION, :RANKHELD)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21miscinfo1
						SET SPECIALTALENTS = :SPECIALTALENTS,CLASSIFICATION = :CLASSIFICATION,RANKHELD = :RANKHELD
			          WHERE SPECIALTALENTS = :SPECIALTALENTS';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21miscinfo1
					  WHERE SPECIALTALENTS = :SPECIALTALENTS AND CLASSIFICATION = :CLASSIFICATION AND RANKHELD = :RANKHELD';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21miscinfo1.SPECIALTALENTS AS 'Special Talents', cin21miscinfo1.CLASSIFICATION AS 'Years of Experience', cin21miscinfo1.RANKHELD AS 'Level of Expertise'
			 FROM #session.companycode#_cbose.cin21miscinfo1 cin21miscinfo1
			 WHERE  cin21miscinfo1.PERSONNELIDNO = '#session.chapa#'
