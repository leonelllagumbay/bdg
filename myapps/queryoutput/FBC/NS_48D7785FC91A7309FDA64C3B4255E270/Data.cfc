...
ExtDirect=true
{
	...
	Usql = "SELECT egintestquery.TESTID, egintestquery.AFLOAT AS 'A Float', egintestquery.ANUMBER AS 'A Number', egintestquery.ASTRING AS 'A String', egintestquery.ATEXT AS 'A Text', egintestquery.ATIME AS 'A Time', egintestquery.ADATETIME AS 'A Date Time'
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'INSERT INTO EGINTESTQUERY (TESTID,AFLOAT,ANUMBER,ASTRING,ATEXT,ATIME,ADATETIME)
		          VALUES (:TESTID, :AFLOAT, :ANUMBER, :ASTRING, :ATEXT, :ATIME, :ADATETIME)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE EGINTESTQUERY
						SET TESTID = :TESTID,AFLOAT = :AFLOAT,ANUMBER = :ANUMBER,ASTRING = :ASTRING,ATEXT = :ATEXT,ATIME = :ATIME,ADATETIME = :ADATETIME
			          WHERE TESTID = :TESTID';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM EGINTESTQUERY
					  WHERE TESTID = :TESTID AND AFLOAT = :AFLOAT AND ANUMBER = :ANUMBER AND ASTRING = :ASTRING AND ATEXT = :ATEXT AND ATIME = :ATIME AND ADATETIME = :ADATETIME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
