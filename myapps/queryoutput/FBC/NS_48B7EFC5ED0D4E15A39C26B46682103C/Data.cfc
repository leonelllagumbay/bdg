...
ExtDirect=true
{
	...
	Usql = "SELECT egintestquery.ADATETIME AS 'A Date Time', egintestquery.AFLOAT AS 'A Float', egintestquery.ANUMBER AS 'A Number', egintestquery.ASTRING AS 'A String', egintestquery.ATEXT AS 'A Text', egintestquery.ATIME AS 'A Time'
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE EGINTESTQUERY
						SET ADATETIME = :ADATETIME,AFLOAT = :AFLOAT,ANUMBER = :ANUMBER,ASTRING = :ASTRING,ATEXT = :ATEXT,ATIME = :ATIME
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM EGINTESTQUERY
					  WHERE ADATETIME = :ADATETIME AND AFLOAT = :AFLOAT AND ANUMBER = :ANUMBER AND ASTRING = :ASTRING AND ATEXT = :ATEXT AND ATIME = :ATIME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
