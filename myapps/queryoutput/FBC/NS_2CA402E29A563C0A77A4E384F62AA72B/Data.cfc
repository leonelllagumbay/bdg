...
ExtDirect=true
{
	...
	if (trim(WHERE) == "") {
		WHERE = "WHERE  cin21chareference.PERSONNELIDNO = '#session.chapa#'";
	} else {
		WHERE = "#WHERE#  AND  cin21chareference.PERSONNELIDNO = '#session.chapa#'";
	}
	...
	Usql = "SELECT cin21chareference.NAME AS 'Name', cin21chareference.COMPANY AS 'Company', cin21chareference.OCCUPATION AS 'Occupation', cin21chareference.ADDRESS AS 'Address', cin21chareference.CELLULARPHONE AS 'Mobile Number', cin21chareference.TELEPHONENUMBER AS 'Telephone Number'
			 FROM #session.companycode#_cbose.cin21chareference cin21chareference
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["cin21chareference-NAME"] = dquery["NAME"][b];
    			tmpresult["cin21chareference-COMPANY"] = dquery["COMPANY"][b];
    			tmpresult["cin21chareference-OCCUPATION"] = dquery["OCCUPATION"][b];
    			tmpresult["cin21chareference-ADDRESS"] = dquery["ADDRESS"][b];
    			tmpresult["cin21chareference-CELLULARPHONE"] = dquery["MOBILE NUMBER"][b];
    			tmpresult["cin21chareference-TELEPHONENUMBER"] = dquery["TELEPHONE NUMBER"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="NAME",value="#args[b]['cin21chareference-NAME']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="COMPANY",value="#args[b]['cin21chareference-COMPANY']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="OCCUPATION",value="#args[b]['cin21chareference-OCCUPATION']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="ADDRESS",value="#args[b]['cin21chareference-ADDRESS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="CELLULARPHONE",value="#args[b]['cin21chareference-CELLULARPHONE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="TELEPHONENUMBER",value="#args[b]['cin21chareference-TELEPHONENUMBER']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO {COMPANYCODE}_cbose.cin21chareference (NAME,COMPANY,OCCUPATION,ADDRESS,CELLULARPHONE,TELEPHONENUMBER)
		          VALUES (:NAME, :COMPANY, :OCCUPATION, :ADDRESS, :CELLULARPHONE, :TELEPHONENUMBER)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE {COMPANYCODE}_cbose.cin21chareference
						SET NAME = :NAME,COMPANY = :COMPANY,OCCUPATION = :OCCUPATION,ADDRESS = :ADDRESS,CELLULARPHONE = :CELLULARPHONE,TELEPHONENUMBER = :TELEPHONENUMBER
			          WHERE NAME = :NAME';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM {COMPANYCODE}_cbose.cin21chareference
					  WHERE NAME = :NAME AND COMPANY = :COMPANY AND OCCUPATION = :OCCUPATION AND ADDRESS = :ADDRESS AND CELLULARPHONE = :CELLULARPHONE AND TELEPHONENUMBER = :TELEPHONENUMBER';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
remote struct function exportGridToExcel(required String querycode, required String querytitle) ExtDirect=true {
	...
	Usql = "SELECT cin21chareference.NAME AS 'Name', cin21chareference.COMPANY AS 'Company', cin21chareference.OCCUPATION AS 'Occupation', cin21chareference.ADDRESS AS 'Address', cin21chareference.CELLULARPHONE AS 'Mobile Number', cin21chareference.TELEPHONENUMBER AS 'Telephone Number'
			 FROM #session.companycode#_cbose.cin21chareference cin21chareference
			 WHERE  cin21chareference.PERSONNELIDNO = '#session.chapa#'
