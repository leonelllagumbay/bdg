...
ExtDirect=true
{
	...
	Usql = "SELECT eglkpmskeyresultarea.KEYRESULTAREACODE AS 'Key Result Area Code', eglkpmskeyresultarea.KEYRESULTAREAS AS 'Key Result Area', eglkpmskeyresultarea.OBJECTIVE AS 'Objective', eglkpmskeyresultarea.KEYRESULTINDICATOR AS 'Key Result Indicator'
			 FROM ibose_global.eglkpmskeyresultarea eglkpmskeyresultarea
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["eglkpmskeyresultarea-KEYRESULTAREAS"] = dquery["KEY RESULT AREA"][b];
    			tmpresult["eglkpmskeyresultarea-OBJECTIVE"] = dquery["OBJECTIVE"][b];
    			tmpresult["eglkpmskeyresultarea-KEYRESULTINDICATOR"] = dquery["KEY RESULT INDICATOR"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="KEYRESULTAREACODE",value="#args[b]['eglkpmskeyresultarea-KEYRESULTAREACODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="KEYRESULTAREAS",value="#args[b]['eglkpmskeyresultarea-KEYRESULTAREAS']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="OBJECTIVE",value="#args[b]['eglkpmskeyresultarea-OBJECTIVE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="KEYRESULTINDICATOR",value="#args[b]['eglkpmskeyresultarea-KEYRESULTINDICATOR']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.eglkpmskeyresultarea (KEYRESULTAREACODE,KEYRESULTAREAS,OBJECTIVE,KEYRESULTINDICATOR)
		          VALUES (:KEYRESULTAREACODE, :KEYRESULTAREAS, :OBJECTIVE, :KEYRESULTINDICATOR)';
		queryService.setDatasource(session.global_dsn);
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.eglkpmskeyresultarea
						SET KEYRESULTAREACODE = :KEYRESULTAREACODE,KEYRESULTAREAS = :KEYRESULTAREAS,OBJECTIVE = :OBJECTIVE,KEYRESULTINDICATOR = :KEYRESULTINDICATOR
			          WHERE KEYRESULTAREACODE = :KEYRESULTAREACODE';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.eglkpmskeyresultarea
					  WHERE KEYRESULTAREACODE = :KEYRESULTAREACODE AND KEYRESULTAREAS = :KEYRESULTAREAS AND OBJECTIVE = :OBJECTIVE AND KEYRESULTINDICATOR = :KEYRESULTINDICATOR';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
