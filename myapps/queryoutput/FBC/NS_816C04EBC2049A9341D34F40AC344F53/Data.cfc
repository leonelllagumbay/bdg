...
ExtDirect=true
{
	...
	Usql = "SELECT glkcourse.COURSECODE AS 'Course Code', glkcourse.DESCRIPTION AS 'Description'
			 FROM ibose_global.glkcourse glkcourse
			 #WHERE#
	...
	for (var b = start+1; b <= start+limit; b++) {
    	tmpresult = StructNew();
    			...
    			tmpresult["glkcourse-COURSECODE"] = dquery["COURSE CODE"][b];
    			tmpresult["glkcourse-DESCRIPTION"] = dquery["DESCRIPTION"][b];
...
public struct function Create(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		queryService = new query();
		queryService.addParam(name="COURSECODE",value="#args[b]['glkcourse-COURSECODE']#",cfsqltype="CF_SQL_VARCHAR");
			queryService.addParam(name="DESCRIPTION",value="#args[b]['glkcourse-DESCRIPTION']#",cfsqltype="CF_SQL_VARCHAR");
			
		Usql = 'INSERT INTO ibose_global.glkcourse (COURSECODE,DESCRIPTION)
...
ExtDirect=true
{
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'UPDATE ibose_global.glkcourse
						SET COURSECODE = :COURSECODE,DESCRIPTION = :DESCRIPTION
...
public struct function Destroy(required any args) ExtDirect=true {
	...
	for (b = 1; b <= ArrayLen(args); b++) {
		...
		Usql = 'DELETE FROM ibose_global.glkcourse
					  WHERE COURSECODE = :COURSECODE AND DESCRIPTION = :DESCRIPTION';
		queryService.setDatasource(session.global_dsn);
	  	queryService.setName('qryname');
		queryService.setSQL(Usql);
		var theResultSet = queryService.execute();
