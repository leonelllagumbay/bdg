<cfcomponent name="Navigator" ExtDirect="true">
	...
	<cffunction name="getMyApps" ExtDirect="true" >
		...
		<cfset menuidArr = ArrayNew(1) />

		<cfset menuidres = ORMExecuteQuery("SELECT MENUID FROM EGRTMENU WHERE USERID = '#session.userid#'", false) />
		<cfloop array="#menuidres#" index="a">
			<cfset ArrayAppend(menuidArr, a) />
		</cfloop>

		<cfset menuresGrp = ORMExecuteQuery("SELECT MENUID FROM EGRTMENUGROUP
												   WHERE USERGRPID IN (SELECT USERGRPIDFK FROM EGRGROLEINDEX
														                                         WHERE USERGRPMEMBERSIDX = '#session.userid#')", false) />

		<cfloop array="#menuresGrp#" index="b">
			<cfset ArrayAppend(menuidArr, b) />
		</cfloop>

		<!--- Convert the menu result from array to list compatible with SQL query --->
		<cfset incond = "('" & ArrayToList(menuidArr, "','") & "')"/>

		<cfquery name="qryApps" datasource="#session.global_dsn#" >
			SELECT * FROM EGRGMENU
			 WHERE MENUID IN #PreserveSingleQuotes(incond)#
			ORDER BY HIERARCHYLEVEL DESC, MENUORDER ASC
		</cfquery>
	...
	<cffunction name="getMyWorklist" ExtDirect="true" >
		...
		<cfset dataObj['name'] = "" >
		<cfset dataObj['action'] = "" >
        <cfset dataObj['iconCls'] = "redbackground" >
			<cfset Item2 = ArrayNew(1) >
			<cfloop query="theForms" >
				<cfset dataObj2 = StructNew() >
				<cfset dataObj2['text'] = theForms.EFORMNAME >
				<cfset dataObj2['name'] = "#EFORMID#" >
				<cfset dataObj2['actiontype'] = "getmyeforms" >
				<cfset dataObj2['action'] = "?bdg=MAINUSRAPPF5038527-9F22-9981-A084244087E398BD&eformid=#EFORMID#&actiontype=getmyeforms&myvar=hi" >
				<cfset ArrayAppend(Item2, dataObj2) >
			</cfloop>

		<cfif ArrayLen(Item2) neq 0 >
				...
				<cfset dataObj['iconCls'] = "redbackground" >
					...
					<cfloop query="countFormIDResult" >
						<cfif countFormIDResult.NEW gt 0 >
							...
							<cfset dataObj2['name'] = "#EFORMID#" >
							<cfset dataObj2['actiontype'] = "getneweforms" >
							<cfset dataObj2['action'] = "?bdg=MAINUSRAPPF5038527-9F22-9981-A084244087E398BD&eformid=#EFORMID#&actiontype=getneweforms&myvar=hi" >
							<cfset ArrayAppend(Item2, dataObj2) >
						</cfif>
				...
				<cfset dataObj['action'] = "" >
					...
					<cfloop query="countFormIDResult" >
						<cfif countFormIDResult.PENDING gt 0 >
							...
							<cfset dataObj2['name'] = "#EFORMID#" >
							<cfset dataObj2['actiontype'] = "getpendingeforms" >
							<cfset dataObj2['action'] = "?bdg=MAINUSRAPPF5038527-9F22-9981-A084244087E398BD&eformid=#EFORMID#&actiontype=getpendingeforms&myvar=hi" >
							<cfset ArrayAppend(Item2, dataObj2) >
						</cfif>
					</cfloop>
	...
	<cffunction name="saveShortCut" ExtDirect="true" >
		...
		<cftry>
			...
			<cfset todayTime = TimeFormat(now(),'HH:MM:SS') >
				<cfquery name="insertShortcut" datasource="#session.global_dsn#">
					<cfif trim(thetype) eq "SHORTCUT" >
		              	INSERT INTO EGINSHORTCUT (PERSONNELIDNO,SHORTCUTNAME,SHORTCUTLINK,RECDATECREATED)
		              	  VALUES (
		              	  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#session.chapa#">,
	              	 ...
	              	 <cfelseif trim(thetype) eq "BOOKMARK" >
	              	 	INSERT INTO EGINBOOKMARK (PERSONNELIDNO,BOOKMARKNAME,BOOKMARKLINK,RECDATECREATED)
		              	  VALUES (
		              	  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#session.chapa#">,
		              	  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#thename#">,
		              	  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#thelink#">,
	...
	<cffunction name="getMyShortcut" ExtDirect="true" >
		...
		<cfquery name="qryShortCut" datasource="#session.global_dsn#" >
	...
	<cffunction name="removeShortCut" ExtDirect="true" >
		<cfargument name="thename" required="true" type="string" >
		<cfargument name="thetype" required="true" type="string" >
		<cfquery name="deleteShortcut" datasource="#session.global_dsn#" >
			<cfif trim(thetype) eq "SHORTCUT" >
				DELETE FROM EGINSHORTCUT
				 WHERE PERSONNELIDNO = '#session.chapa#' AND SHORTCUTNAME = '#thename#'
		    <cfelseif trim(thetype) eq "BOOKMARK" >
		    	DELETE FROM EGINBOOKMARK
