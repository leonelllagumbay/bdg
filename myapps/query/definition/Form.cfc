...
<cffunction name="Submit" ExtDirect="true" ExtFormHandler="true">
	<cfscript>
		...
		if(Not IsDefined("form.SORTERSDIRECTION") || form.SORTERSDIRECTION== "" ) dsource.setSORTERSDIRECTION(javacast("null",""));
		else dsource.setSORTERSDIRECTION(trim(form.SORTERSDIRECTION));
		...
		if(Not IsDefined("form.FILTERTYPE") || form.FILTERTYPE== "" ) dsource.setFILTERTYPE(javacast("null",""));
		else dsource.setFILTERTYPE(trim(form.FILTERTYPE));
		...
		if(Not IsDefined("form.STOREAUTOLOAD") || form.STOREAUTOLOAD== "" ) dsource.setSTOREAUTOLOAD(javacast("null",""));
		else dsource.setSTOREAUTOLOAD(trim(form.STOREAUTOLOAD));
		...
		if(Not IsDefined("form.GROUPFIELD") || form.GROUPFIELD== "" ) dsource.setGROUPFIELD(javacast("null",""));
		else dsource.setGROUPFIELD(trim(form.GROUPFIELD));

		if(Not IsDefined("form.PROXYTYPE") || form.PROXYTYPE== "" ) dsource.setPROXYTYPE(javacast("null",""));
		else dsource.setPROXYTYPE(trim(form.PROXYTYPE));

		if(Not IsDefined("form.PROXYREADER") || form.PROXYREADER== "" ) dsource.setPROXYREADER(javacast("null",""));
		else dsource.setPROXYREADER(trim(form.PROXYREADER));

		if(Not IsDefined("form.PROXYWRITER") || form.PROXYWRITER== "" ) dsource.setPROXYWRITER(javacast("null",""));
		else dsource.setPROXYWRITER(trim(form.PROXYWRITER));

		if(Not IsDefined("form.PROXYURL") || form.PROXYURL== "" ) dsource.setPROXYURL(javacast("null",""));
		else dsource.setPROXYURL(trim(form.PROXYURL));

		if(Not IsDefined("form.STOREFIELDS") || form.STOREFIELDS== "" ) dsource.setSTOREFIELDS(javacast("null",""));
		else dsource.setSTOREFIELDS(trim(form.STOREFIELDS));

		if(Not IsDefined("form.FORMGRIDCOMBO") || form.FORMGRIDCOMBO== "" ) dsource.setFORMGRIDCOMBO(javacast("null",""));
		else dsource.setFORMGRIDCOMBO(trim(form.FORMGRIDCOMBO));
...
<cffunction name="Load" ExtDirect="true" returntype="struct">
	...
	if(IsDefined("dsource")) {
		for(i=1; i<=ArrayLen(dsource); i++) {
			...
			tmpresult["SORTERSDIRECTION"] = dsource[i].getSORTERSDIRECTION();
			tmpresult["STOREFILTERS"] = dsource[i].getSTOREFILTERS();
			tmpresult["FILTERTYPE"] = dsource[i].getFILTERTYPE();
			tmpresult["STOREPAGESIZE"] = dsource[i].getSTOREPAGESIZE();
			tmpresult["STORETIMEOUT"] = dsource[i].getSTORETIMEOUT();
			tmpresult["STOREAUTOLOAD"] = dsource[i].getSTOREAUTOLOAD();
			tmpresult["STOREEXTRA"] = dsource[i].getSTOREEXTRA();
			tmpresult["STOREPROXYEXTRA"] = dsource[i].getSTOREPROXYEXTRA();
			tmpresult["GROUPFIELD"] = dsource[i].getGROUPFIELD();
			tmpresult["PROXYTYPE"] = dsource[i].getPROXYTYPE();
			tmpresult["PROXYREADER"] = dsource[i].getPROXYREADER();
			tmpresult["PROXYWRITER"] = dsource[i].getPROXYWRITER();
			tmpresult["PROXYURL"] = dsource[i].getPROXYURL();
			tmpresult["STOREFIELDS"] = dsource[i].getSTOREFIELDS();
			tmpresult["FORMGRIDCOMBO"] = dsource[i].getFORMGRIDCOMBO();
		}
	}
...
<cfscript>
		...
		if(Not IsDefined("form.COLUMNGROUP") || form.COLUMNGROUP == "" ) dcolumn.setCOLUMNGROUP(javacast("null",""));
		else dcolumn.setCOLUMNGROUP(trim(form.COLUMNGROUP));

		if(Not IsDefined("form.ENABLEGROUPHEADER") || form.ENABLEGROUPHEADER == "" ) dcolumn.setENABLEGROUPHEADER(javacast("null",""));
		else dcolumn.setENABLEGROUPHEADER(trim(form.ENABLEGROUPHEADER));
		...
		if(Not IsDefined("form.COLUMNFORMAT") || form.COLUMNFORMAT == "" ) dcolumn.setCOLUMNFORMAT(javacast("null",""));
		else dcolumn.setCOLUMNFORMAT(trim(form.COLUMNFORMAT));
		...
		if(Not IsDefined("form.SUMMARYTYPE") || form.SUMMARYTYPE == "" ) dcolumn.setSUMMARYTYPE(javacast("null",""));
		else dcolumn.setSUMMARYTYPE(trim(form.SUMMARYTYPE));

		if(Not IsDefined("form.SUMMARYRENDERER") || form.SUMMARYRENDERER == "" ) dcolumn.setSUMMARYRENDERER(javacast("null",""));
		else dcolumn.setSUMMARYRENDERER(trim(form.SUMMARYRENDERER));
...
<cffunction name="LoadColumnDetail" ExtDirect="true" returntype="struct">
	...
	if(IsDefined("dcolumn")) {
		for(i=1; i<=ArrayLen(dcolumn); i++) {
			...
			tmpresult["COLUMNGROUP"] = dcolumn[i].getCOLUMNGROUP();
			tmpresult["ENABLEGROUPHEADER"] = dcolumn[i].getENABLEGROUPHEADER();
			...
			tmpresult["COLUMNFORMAT"] = dcolumn[i].getCOLUMNFORMAT();
			tmpresult["COLUMNEXTRA"] = dcolumn[i].getCOLUMNEXTRA();
			tmpresult["SUMMARYTYPE"] = dcolumn[i].getSUMMARYTYPE();
			tmpresult["SUMMARYRENDERER"] = dcolumn[i].getSUMMARYRENDERER();
		}
	}
