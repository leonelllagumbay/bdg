...
<cffunction name="generateGrid" ExtDirect="true">
	...
	<cftry>
		...
		<cfset eFormName = ORMExecuteQuery("SELECT EFORMNAME, FORMFLOWPROCESS FROM EGRGEFORMS WHERE EFORMID = '#eformid#'", true ) >
		...
		<cfset formflowprocess = eFormName[2] />
		<cfset eFormName = eFormName[1] />
		...
		<cfloop array="#formTableData#" index="tableIndex" >
			...
			<cfloop array="#tableColumnData#" index="columnIndex" >
				...
				<cfif XTYPE EQ "grid" OR XTYPE EQ "signature" OR XTYPE EQ "commentfield">
					<cfcontinue>
				</cfif>
	...
	var autoeFormGrid = Ext.create('Ext.grid.Panel', {
		...
		xtype: 'tbseparator'
	  	...
	  	handler: function(btn) {
	  		...
	  		if(selectedRecord) {
	  			...
	  			Ext.Msg.alert('Message','Please select a record first!');
	  		}
	  	}
	  },{
	  	xtype: 'tbseparator'
	  	...
	  	handler: function(btn) {
	  		...
	  		if(status == 'N' || status == 'R') {
	  			...
	  			myMask.hide();
	  			Ext.Msg.alert('Message','Please copy this form first to edit!');
	  		}
	  	} else {
	  		Ext.Msg.alert('Message','Please select a record first!');
	  	}
		...
		handler: function(btn) {
			...
			if(selectedRecords.length > 0) {
				Ext.Msg.show({
	       			title: '',
	       			msg: 'You are about to delete an eForm which has processed records. \n Would you like to continue?',
		...
		handler: function(btn) {
			...
			if (selection.length != 0) {
				...
				Ext.Msg.show({
	       			title: 'Route selected ' + eform + '?',
	       			msg: 'Only eForms with status \'NEW\' will be routed.',
	       			buttons: Ext.Msg.YESNO,
		    	    	...
		    	    	if (res == 'yes') {
							...
							for (var cntr=0; cntr<selection.length; cntr++) {
								...
								if(status == 'N' || status == '') {
									...
									if ('#formflowprocess#' == 'NA') {
										Ext.create('Ext.window.Window', {
										    title: 'Select Form Flow Process',
										    width: 420,
										    modal: true,
										    layout: 'form',
										    buttonAlign: 'center',
										    items: [{
										    	...
										    	items: [{
													...
													store: Ext.create('Ext.data.Store', {
														fields: ['mainprocessname','mainprocesscode'],
														autoLoad: false,
														proxy: {
															type: 'direct',
															timeout: 300000,
										    ...
										    buttons: [{
										    	...
										    	text: 'Continue',
										    	handler: function(b) {
													var df = b.up('window').down('form').getForm();
													if (df.isValid()) {
													    var cval = df.getValues();
													    dformflowprocess = cval['formflow'];
													    b.up('window').close();
													    myMask.show();
													    Ext.ss.active.startRoute(eformid,processid,theLevel,thetable,dformflowprocess,function(result) {
															myMask.hide();
															if(result == 'success') {
																Ext.Msg.show({
																	title: '',
									...
									Ext.ss.dynamicApprover.isDynamic('1',eformid, function(result){
										if(result.HASDYNAMIC == true) {
											...
											Ext.ss.active.startRoute(eformid,processid,theLevel,thetable,'NA', function(result) {
												myMask.hide();
												if(result == 'success') {
													Ext.Msg.show({
														title: '',
					...
					Ext.Msg.alert('Message','No form selected. Please select an eForm to route.');
				}
			}
		},{
			xtype: 'tbseparator'
			...
			handler: function(btn) {
					...
					Ext.ss.active.generateMap(eformid,processid,pidno,theLevel,theTable, function(result) {
						...
						try {
							...
							if(result == 'cannotviewroutemap') {
								Ext.Msg.alert('Message','Unable to view status');
							} else {
								Ext.Msg.alert('Form flow is undefined.');
								console.log(result);
							}
						}
					});
				...
				Ext.Msg.alert('Message','No form selected. Please select an eForm first.');
			}
		}
	},{
		xtype: 'tbseparator'
	    	    ...
	    	    fn: function(res, txt) {
	    	    	if (res == 'yes') {
						...
						if(selectedRecord.length > 0) {
							...
							Ext.Msg.alert('Message','Please select at least one record to approve.');
						}
	    	    	}
	    	    }
       		});
		...
		<cfloop array="#formTableData#" index="tableIndex" >
			...
			<cfloop array="#tableColumnData#" index="columnIndex" >
				...
				<cfif trim(YPOSITION) EQ "" >
					<cfset YPOSITION = "" >
					<cfset YPOSITIONOFFSET = "" >
				<cfelse>
					<cfset YPOSITIONOFFSET = "y: #val(YPOSITION)+27#," >
				...
				<cfelseif XTYPE EQ "filefield"> <!---filefield--->
					...
					<cfsavecontent variable="groupItems" >
						<cfoutput>
							{	xtype: 'filefield',id: #INPUTID#, fieldLabel: '#FIELDLABEL#',name: 'file#fileCount#',
								#XPOSITION##YPOSITION##ANCHORPOSITION# hidden: #ISHIDDEN#, labelAlign: '#FIELDLABELALIGN#',
								#FIELDLABELWIDTH# allowBlank: #ALLOWBLANK#, cls: '#CSSCLASS#', disabled: #ISDISABLED#,
								#HEIGHT# #WIDTH# minLength: #MINCHARLENGTH#,
								maxLength: #MAXCHARLENGTH#,
								...
								#VALIDATIONTYPE# #VTYPETEXT#
								fieldStyle: '#INPUTFORMATB#',
								readOnly: #ISREADONLY#,value: '#INPUTVALUE#'
							},{ xtype: 'hiddenfield',//for file////please do not remove
								name: '#LEVELID#__#TABLENAME#__#COLUMNNAME#',value: '',fieldLabel: '#FIELDLABEL#',
								#XPOSITION#
								#YPOSITION#
								#ANCHORPOSITION#  hidden: #ISHIDDEN#, labelAlign: '#FIELDLABELALIGN#',
								...
								xtype: 'hiddenfield',
								name: '#LEVELID#__#TABLENAME#__#COLUMNNAME#__FILE__#fileCount#',
								allowBlank: true
							}
						</cfoutput>
					</cfsavecontent>
				...
				<cfelseif XTYPE EQ "commentfield" > <!---comment field, get finite comments specific to this eform--->
					<cfsavecontent variable="dirStore" >
							<cfoutput>
								var #TABLENAME##COLUMNNAME# = Ext.create('Ext.data.Store', {
									...
									pageSize: 40,
									listeners: {
										load: function(thiss,rec,eopts) {
											console.log('data directory changed');
											console.log(rec);
										}
									},
									proxy: {
										type: 'direct',
										timeout: 300000,
										directFn: 'Ext.ss.lookup.getDirectoryList',
										paramOrder: ['limit', 'page', 'query', 'start'],
										reader: {
											root: 'topics',
											totalProperty: 'totalCount'
						...
						<cfset dirStore = rereplace(dirStore,"[\t\n\f\r]","","all") />
						<cfset columnModelStore[cntStore] = dirStore >
						...
						<cfoutput>
							{
							    ...
							    action: '#LEVELID#__#TABLENAME#__PERSONNELIDNO',
								#ANCHORPOSITION# hidden: #ISHIDDEN#,
								cls: '#CSSCLASS#', #HEIGHT# #WIDTH#
								border: '#BORDER#',
								style: #STYLE#,
								autoScroll: true,
								items: [{
									...
									items: [{
										...
										handler: function(dbtn) {
											var btncontainer = dbtn.up('container');
									     	...
									     	var fg = Ext.getCmp('#eformid#');
											var fs = fg.getSelectionModel().getSelection()[0];
											if (!fs.data) return false;
											var deformid = '#eformid#';
											var downerpid = fs.data['#LEVELID#__#TABLENAME#__PERSONNELIDNO'];
									     	if(commentval.trim().length > 0) {
     											dbtn.setDisabled(true);
												Ext.ss.Complements.saveMyComment(deformid,downerpid,commentval, function(resp) {
									     			if(resp.issuccess == true) {
										     			dbtn.setDisabled(false);
										     			htmled.setValue('');
										     			var dp = btncontainer.up('panel');
										     			var op = {
										     				xtype: 'container',
										     				padding: 7,
										     				flex: 1,
								...
								listeners: {
									...
									try {
										...
										Ext.ss.Complements.loadMyComments(deformid,downerpid,forumstartnumref, function(resp) {
									    	p.getEl().unmask();
									    	if(resp.length == 0) {
									    		forumstartnumref = -1;
									    		return true;
				...
				<cfelseif XTYPE EQ "directoryfield" > <!---directory field, get files from efiles--->
					<cfsavecontent variable="dirStore" >
							<cfoutput>
								var #TABLENAME##COLUMNNAME# = Ext.create('Ext.data.Store', {
									fields: ['displayname', 'codename'],
						...
						<cfoutput>
							{
								...
								valueField: 'codename',disabled: #ISDISABLED#,
								queryMode: 'remote',
								...
								listeners: {
							        change: function(d, n, o) {
							        	var ddir = d.up('form').down('displayfield[name=directory#LEVELID#__#TABLENAME#__#COLUMNNAME#]');
							        	var myMask = Ext.create('Ext.LoadMask',{
											target: ddir,
											msg: "Please wait..."
										});
										myMask.show();
							        	Ext.ss.lookup.getTheFiles(n, function(res) {
							        	    myMask.hide();
							        	    d.removeListener('change', d);
							        		var dval = res.filedisplay;
							        		var ddir = d.up('form').down('displayfield[name=directory#LEVELID#__#TABLENAME#__#COLUMNNAME#]');
							            	ddir.setValue(dval);
							        	});
							        }
								...
								#FIELDLABELWIDTH#
								allowBlank: #ALLOWBLANK#,
								cls: '#CSSCLASS#',#HEIGHT# #WIDTH# minLength: #MINCHARLENGTH#, maxLength: #MAXCHARLENGTH#,
								margin: #MARGIN#,
								padding: #PADDING#,
								border: '#BORDER#',
								style: #STYLE#, id: #INPUTID#,readOnly: #ISREADONLY#,value: '#INPUTVALUE#',
								#VALIDATIONTYPE# #VTYPETEXT# fieldStyle: '#INPUTFORMATB#'
							 },{
							 	xtype: 'displayfield',
							 	labelAlign: 'right',
							 	fieldLabel: ' ',
							 	name: 'directory#LEVELID#__#TABLENAME#__#COLUMNNAME#',
							 	#FIELDLABELWIDTH#
							 	#XPOSITION# #YPOSITIONOFFSET# #ANCHORPOSITION# hidden: #ISHIDDEN#,
							 	margin: #MARGIN#,
								padding: #PADDING#,
								border: '#BORDER#', style: #STYLE#,
								value: ' '
								    		...
								    		Ext.Msg.prompt('Copy Selection', 'Please enter the destination\'s document number:', function(resp, text){
											    if (resp == 'ok'){
											    	...
											    	if(selData.length > 0) {
											    		...
											    		Ext.Msg.alert('Message','No selection.');
											    	}
											    	return true;
										    	}
											});
...
<cfsavecontent variable="auxScript" >
	<cfoutput>
		...
		defaultType: 'textfield'/*NOTFOREQUERY*/,
		api: {
			...
			handler: function(thiss) {
				...
				if(theForm.isValid()) {
					theForm.submit({
			   		...
			   		Ext.Msg.alert('Message','Please provide correct value for each field!');
			 }
		}
	},{
				...
				Ext.Msg.show({
		    	    ...
		    	    fn: function(res, txt) {
		    	    	if (res == 'yes') {
							...
							if(approved == 'R') {
								Ext.Msg.alert('Message','The form cannot be returned to the originator because he or she has no reponse yet.');
								return false;
							}
							if(theForm.isValid()) {
								theForm.submit({
					...
					Ext.ss.active.generateMap(eformid,processid,pidno,theLevel,theTable, function(result) {
						...
						try {
							...
							if(result == 'cannotviewroutemap') {
								...
								Ext.Msg.alert('Message','Form flow is undefined.');
							}
						}
					});
				} else {
