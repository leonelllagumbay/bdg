<cfcomponent name="defdata" ExtDirect="true">  
...
<cfset eformData.setPERSONNELIDNO("#session.chapa#") >
...
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__EFORMID__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__PROCESSID__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__ACTIONBY__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__PERSONNELIDNO__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__RECDATECREATED__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__DATEACTIONWASDONE__X' >
<cfset ArrayAppend(columnNameModel, colModel) >
<cfset colModel = firstlevel & '__' & firsttable & '__DATELASTUPDATE__X' >
<cfset ArrayAppend(columnNameModel, colModel) > 
...
<cfset theQuery = "SELECT #theSelection# 
					FROM #theTable#
				   WHERE #theCondition# #theFilterCondition# AND #firsttable#.PERSONNELIDNO = '#session.chapa#'
				   ORDER BY #ORDERBY#">
<cfelseif isnew eq "true" >
	<cfset statusFlag = "neweforms" >
...
<cfelseif ispending eq "true" > <!---get pending forms using the current PID---> 
	<cfset statusFlag = "pendingeforms" >
	<cfset theQuery = "SELECT #theSelection# 
						FROM #theTable#
					   WHERE #theCondition# #theFilterCondition# AND #firsttable#.PERSONNELIDNO = '#session.chapa#'
					   ORDER BY #ORDERBY#">
...
<!--- end generate script --->
	...
	<!---using this total count, synchronize the table on total count w/c is egineformcount--->
		<cfif statusFlag eq "neweforms" >
			<cfset updateCount = EntityLoad("EGINEFORMCOUNT", { EFORMID =#eformid#, PERSONNELIDNO = #session.chapa#}, true ) >
			<cfif isdefined("updateCount") > 			
			
				<cfset updateCount.setNEW(qryDynamic.recordcount) >
				<cfset EntitySave(updateCount) >  
				<cfset ormflush()>
			<cfelse>
			</cfif> 	
		<cfelseif statusFlag eq "pendingeforms" >
			<cfset updateCount = EntityLoad("EGINEFORMCOUNT", { EFORMID =#eformid#, PERSONNELIDNO = #session.chapa#}, true ) >
			<cfif isdefined("updateCount") >	
				<cfset updateCount.setPENDING(qryDynamic.recordcount) >
				<cfset EntitySave(updateCount) >  
				<cfset ormflush()>
			<cfelse>
			</cfif> 	
		...
		<cfset tmpresult = StructNew() > <!---Creates new structure in every loop to be added to the array--->
			...
			<cfif getMainTableID[4] eq "true" >
				<cfloop array="#columnNameModel#" index="outIndex" > 
					<cfset listInd = ListDeleteAt(outIndex,ListLen(outIndex,"__"),'__') >
					<cftry>
						<cfset tmpresult['#listInd#']      = decrypt(evaluate(listInd), session.ek)  >
					<cfcatch>
				...
				<cfset tmpresult['thefilepath']      = "#session.domain#unDB/forms/#session.companycode#/"  >
			<cfelse>
				<cfloop array="#columnNameModel#" index="outIndex" >
					<cfset listInd = ListDeleteAt(outIndex,ListLen(outIndex,"__"),'__') >
					<cfset tmpresult['#listInd#']      = evaluate(listInd)  >
