...
<cfif ArrayLen(selectArray) gt 0 >
	...
	<cfloop from="1" to="#ArrayLen(columnAutoGenCol)#" index="cntr" >
		...
		<cfset newVal = replace(newVal,"{TIMEFORMAT}", timeformat(now(),"HH:MM:SS"), "all") >
		...
		<cfset newVal = replace(newVal,"{USERID}", session.userid, "all") >
		<cfset newVal = replace(newVal,"{GUID}", session.guid, "all") >
...
<cftry>
	<cfset returnStruct = StructNew() >
	<cfinvoke component="IBOSE.application.Util" method="uploadFileMultiple" returnvariable="arrServerFile">
...
<cffunction name="saveFormNoFile" >
	...
	<cfif Not StructIsEmpty(form) >
		...
		<cftry>
			...
			<cfinvoke method="appendToFormUploadedFiles" >
...
<cffunction name="appendToFormUploadedFiles" access="public" >
	<!--- insert uploaded files in the form struct --->
	<cfloop from="1" to="#ArrayLen(arrServerFile)#" index="dindex">
	    <cfif trim(arrServerFile[dindex]) neq "" >
			<cfloop array="#arrForm#" index="findex" >
				<cfif find('__FILE__#dindex#',findex)>
					<cfset thisfileform = replace(findex,"__FILE__#dindex#","") >
					<cfset "form.#thisfileform#" = arrServerFile[dindex] >
				</cfif>
			</cfloop>
		</cfif>
	</cfloop>
...
<cfloop array="#getApprovers#" index="approverIndx">
	...
	<cfset approvedValue = EntityLoad("EGINROUTERAPPROVERS", eginapproverdata.getAPPROVERSIDFK(), true ) >
	<cfset eginapproverdata.setACTIONCUSTOM(trim(approvedValue.getAPPROVEDVALUE())) >
...
<cfloop array="#getApprovers#" index="approverIndx">
	...
	<cfset approvedValue = EntityLoad("EGINROUTERAPPROVERS", eginapproverdata.getAPPROVERSIDFK(), true ) >
	<cfset eginapproverdata.setACTIONCUSTOM(trim(approvedValue.getDISAPPROVEDVALUE())) >
