...
<cfset valueArr    = ArrayNew(1) >
	...
	<cfset thecontent = thecontent & "<img style='margin: 20px;' src='./resource/image/companylogo/#session.site_ibose#'>" >

	<cfquery name="drouter" datasource="#session.global_dsn#">
		SELECT ROUTERDETAILSID, ROUTERORDER, DESCRIPTION FROM EGINROUTERDETAILS
		WHERE PROCESSIDFK = '#processid#'
		ORDER BY ROUTERORDER ASC;
	</cfquery>
	<cfloop query="drouter">
		<cfquery name="dapprover" datasource="#session.global_dsn#">
			SELECT ACTIONCUSTOM, APPROVERORDER, PERSONNELIDNO FROM EGINAPPROVERDETAILS
			WHERE ROUTERIDFK = '#drouter.ROUTERDETAILSID#'
			ORDER BY APPROVERORDER ASC;
		</cfquery>
		<cfset thecontent = thecontent & "<p><b>#drouter.ROUTERORDER#. #drouter.DESCRIPTION#</b></p>">

		<cfset thecontent = thecontent & "<ul>" >
		<cfloop query="dapprover">
		    <cfquery name="duser" datasource="#session.company_dsn#">
				SELECT FIRSTNAME, LASTNAME, GUID FROM CMFPA
				WHERE PERSONNELIDNO = '#dapprover.PERSONNELIDNO#';
			</cfquery>
			<cfquery name="duserM" datasource="#session.global_dsn#">
				SELECT USERID FROM EGRGUSERMASTER
				WHERE GUID = '#duser.GUID#';
			</cfquery>
			<cfset thecontent = thecontent & "<li>#dapprover.APPROVERORDER# #duser.FIRSTNAME# #duser.LASTNAME# (#duserM.USERID#) - <b>#dapprover.ACTIONCUSTOM#</b> <img alt='Signed: #duserM.USERID#' src='#session.image_path#/Signatures/#duserM.USERID#.jpg'></li>">
		</cfloop>
		<cfset thecontent = thecontent & "</ul><br>" >
	</cfloop>
