...
<cfinvoke component="IBOSE.administrator.Email" method="setUpMailAttr" returnvariable="acoll">
	<cfinvokeargument name="ms" value="#url.ms#">
	<cfinvokeargument name="mu" value="#url.mu#">
	<cfinvokeargument name="mp" value="#url.mp#">
	<cfinvokeargument name="mt" value="#url.mt#">
	<cfinvokeargument name="mpt" value="#url.mpt#">
	<cfinvokeargument name="mtls" value="#url.mtls#">
	<cfinvokeargument name="mssl" value="#url.mssl#">
</cfinvoke>
...
<cfif ArrayLen(freqArray) neq 0 >
	...
	<cftry>
		...
		<cfschedule
			...
			startdate   	 = "#dateformat(dateadd( 'd',-1,now()), 'mm/dd/yy')#"
			starttime   	 = "#timeformat(now(), 'short')#"
			enddate   	     = "#dateformat(endDateB, 'mm/dd/yy')#"
			url        	 	 = "#domain#myapps/form/simulator/schedule.cfm?eformid=#eformid#&processid=#processid#&action=followupemail&domain=#url.domain#&routerid=#therouterdetailsid#&companydsn=#urn.companydsn#&globaldsn=#url.globaldsn#&companyname=#url.companyname#&subcomdsn=#url.subcomdsn#&querydsn=#url.querydsn#&transactiondsn=#url.transactiondsn#&sitedsn=#url.sitedsn#&ms=#URLEncodedFormat(ms)#&mu=#URLEncodedFormat(mu)#&mp=#URLEncodedFormat(mp)#&mt=#mt#&mpt=#mpt#&mtls=#mtls#&mssl=#mssl#"
			requestTimeOut	 = "300"
	...
	<cfmail
			...
			<table border="1" cellpadding="3" cellspacing="1" style="font: 13px Arial; border-collapse: collapse;" width = "550">
				...
				<cfloop from="1" to="#ArrayLen(valueArr)#" index="propIndex" >
					<cfif (propIndex MOD 2) neq 0>
						<TR style="background: ##d3e1f1">
				    <cfelse>
				    	<TR>
				    </cfif>
						<cftry>
