<cfcomponent name="active" ExtDirect="true">
	...
	<cffunction name="startRoute" ExtDirect="true">
		...
		<cfargument name="dformflowprocess" >
		...
		<cftry>
			...
			<!--- Check if the user has selected a form flow process --->
			<cfset dformflowprocess = trim(dformflowprocess) />
			<cfif dformflowprocess eq "NA" >
				<cfset dformflowprocess = formFlowProcess[1] />
			<cfelse>

			</cfif>

			<cfset formProcessData = ArrayNew(1) />
			<cfset formProcessData = EntityLoad("EGINFORMPROCESS", #dformflowprocess#) />
