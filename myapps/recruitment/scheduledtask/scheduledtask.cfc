<cfcomponent displayname="scheduledtask"
hint="Action to take when an applicant is inactive in a company"
output="false"
>
...
<cffunction name="release"
			...
			returntype="void"
>
<cfargument name="referencecode">

	<cfquery name="releaseApp" datasource="#globaldsn#">
        UPDATE EGMFAP
           SET COMPANYCODE            = ''
...
<cffunction name="copytogrgpool"
			access="public"
			returntype="string"
>
<cfargument name="theguid"  >
<cfargument name="applicantnumber" >
<cfargument name="companydsn" >


<cfquery name="getNEOdate" datasource="#companydsn#" >
	SELECT DATENEO
	  FROM ecinonboardchecklist
	 WHERE APPLICANTNUMBER = '#applicantnumber#'
</cfquery>
<cfif isdefined("getNEOdate") >
	<cfif getNEOdate.recordcount GT 0 >
		<cfif len(getNEOdate.DATENEO) GT 7 >
			<cftry>
				<cfschedule
				    action = "delete"
				    task   = "#companydsn##applicantnumber#"
					group  = "RECRUITMENT">
			<cfcatch>
			</cfcatch>
			</cftry>
			<cfreturn "success" > <!---do not make a copy of local records to global--->
		<cfelse>
...
<cfquery name="updateCMFAP" datasource="#companydsn#">
    UPDATE CMFAP
       SET DATELASTUPDATE  =  <cfqueryparam cfsqltype="cf_sql_date" value="#dateformat(now(), 'YYYY-MM-DD')#"/>
     WHERE  GUID = '#theguid#'
</cfquery>
...
<!---revert start--->
<cfset sourceDir = ExpandPath("../../../unDB/forms/#companycode#/")>
...
<!---Revert to EGMFAP-------------------------------------------------------------------------------------->
	<cfquery name="query_from_CMFAP" datasource="#companydsn#" maxrows="1">
	  SELECT   *
		FROM CMFAP
	   WHERE GUID = '#theguid#';
	</cfquery>
 ...
 <cfloop query="query_from_CMFAP">
   ...
   <cfquery name="insertToRecruitmentGT" datasource="#globaldsn#">
...
<cfquery name="query_from_CIN21PERSONALINFO" datasource="#companydsn#" maxrows="1">
  SELECT   *
...
<!---FETCH TO CIN21POSITNAPLD-------------------------------------------------------------------------------------->
  <cfquery name="query_from_CIN21POSITNAPLD" datasource="#companydsn#">
	  SELECT *
...
<cfquery name="query_from_CIN21FAMILYBKGRND" datasource="#companydsn#">
  SELECT   *
...
<cfquery name="query_from_CIN21EDUCATION" datasource="#companydsn#">
  SELECT   *
	FROM CIN21EDUCATION
   WHERE GUID = '#theguid#';
</cfquery>
...
<cfquery name="query_from_CIN21EMPEXTRA" datasource="#companydsn#">
  SELECT   *
	FROM CIN21EMPEXTRA
   WHERE GUID = '#theguid#';
</cfquery>
...
<cfquery name="query_from_CIN21EXAMPASS" datasource="#companydsn#">
  SELECT   *
	FROM CIN21EXAMPASS
   WHERE GUID = '#theguid#';
</cfquery>
...
<cfquery name="query_from_CIN21ACHIEVEMENTS" datasource="#companydsn#">
  SELECT   *
...
<cfquery name="query_from_CIN21MEDHISTORY" datasource="#companydsn#">
  SELECT   *
...
<cfquery name="query_from_CIN21TRAINING" datasource="#companydsn#">
  SELECT   *
	FROM CIN21TRAINING
   WHERE GUID = '#theguid#';
</cfquery>
...
<!---FETCH TO CIN21WORKHISTORY-------------------------------------------------------------------------------------->
   <cfquery name="query_from_CIN21WORKHISTORY" datasource="#companydsn#">
	  SELECT   *
...
<cfquery name="query_from_CIN21RELATIVE" datasource="#companydsn#">
  SELECT   *
	FROM CIN21RELATIVE
   WHERE GUID = '#theguid#';
</cfquery>
...
<cfquery name="query_from_CIN21MISCINFO1" datasource="#companydsn#">
  SELECT   *
	FROM CIN21MISCINFO1
   WHERE GUID = '#theguid#';
</cfquery>
...
<cfquery name="query_from_CIN21CHARREFERENCE" datasource="#companydsn#">
  SELECT   *
...
<cfquery name="query_from_CIN21EMPVIOL" datasource="#companydsn#">
  SELECT   *
	FROM CIN21EMPVIOL
   WHERE GUID = '#theguid#';
</cfquery>
<cfloop query="query_from_CIN21EMPVIOL">
 ...
 <!--- ECINEXAMRESULT --->
 <cfquery name="query_from_ECINEXAMRESULT" datasource="#companydsn#">
  SELECT   *
	FROM ECINEXAMRESULT
   WHERE APPLICANTNUMBER = '#APPLICANTNUMBER#';
</cfquery>
<cfloop query="query_from_ECINEXAMRESULT">
    <cfquery name="insertToMRFStatusTable" datasource="#globaldsn#">
...
<!--- ECINTERVIEWRESULT --->
 <cfquery name="query_from_ECINTERVIEWRESULT" datasource="#companydsn#">
  SELECT   *
...
<!--- ECINTERVIEWRESULTFD --->
 <cfquery name="query_from_ECINTERVIEWRESULTFD" datasource="#companydsn#">
  SELECT   *
...
<!--- ECINTERVIEWRESULTSD --->
 <cfquery name="query_from_ECINTERVIEWRESULTSD" datasource="#companydsn#">
  SELECT   *
...
<!--- ECINTERVIEWRESULTFINAL --->
 <cfquery name="query_from_ECINTERVIEWRESULTFINAL" datasource="#companydsn#">
  SELECT   *
...
<!--- ECINJOBOFFER --->
 <cfquery name="query_from_ECINJOBOFFER" datasource="#companydsn#">
  SELECT   *
	FROM ECINJOBOFFER
   WHERE APPLICANTNUMBER = '#APPLICANTNUMBER#';
</cfquery>
...
<!--- CINPREEMPREQCHKLIST --->
 ...
 <cfquery name="query_from_CINPREEMPREQCHKLIST" datasource="#companydsn#">
  SELECT *
...
<cfloop  query="query_from_CINPREEMPREQCHKLIST">
    ...
    <cfquery name="insertToMRFStatusTableG" datasource="#globaldsn#">
...
<!--- ECINCONTRACT --->
 <cfquery name="query_from_ECINCONTRACT" datasource="#companydsn#">
  SELECT   *
	FROM ECINCONTRACT
   WHERE APPLICANTNUMBER = '#APPLICANTNUMBER#';
</cfquery>
...
<!--- ECINONBOARDCHECKLIST --->
 <cfquery name="query_from_ECINONBOARDCHECKLIST" datasource="#companydsn#">
  SELECT   *
	FROM ECINONBOARDCHECKLIST
   WHERE APPLICANTNUMBER = '#APPLICANTNUMBER#';
</cfquery>
<cfloop query="query_from_ECINONBOARDCHECKLIST">
   ...
   <cftry>
	<cfquery name="removeLocalData" datasource="#companydsn#">
    ...
    <cfquery name="removeGlobalData" datasource="#companydsn#">
...
<!---revert end--->
	...
	<cftry>
		...
		<cfquery name="getMaxApplHoldTime" datasource="#globaldsn#" maxrows="1">
			SELECT CONFIGVALUE
...
<cfcatch>
    ...
    <cfquery name="removeGlobalData" datasource="#globaldsn#">
